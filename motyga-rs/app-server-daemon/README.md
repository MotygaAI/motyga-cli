# motyga-app-server-daemon

> `motyga-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`motyga-app-server-daemon` backs the machine-readable `motyga app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Motyga instances launched over SSH, including fresh developer
...
```sh
motyga app-server daemon start
motyga app-server daemon restart
motyga app-server daemon enable-remote-control
motyga app-server daemon disable-remote-control
motyga app-server daemon stop
motyga app-server daemon version
motyga app-server daemon bootstrap --remote-control
```
...
$HOME/.motyga/packages/standalone/current/motyga app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `MOTYGA_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Motyga is installed through `install.sh` and always launches
the standalone managed binary under `MOTYGA_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `MOTYGA_HOME/packages/standalone/current/motyga` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `MOTYGA_HOME/packages/standalone/current/motyga` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `motyga remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `MOTYGA_HOME`, so a concurrent
...
The daemon stores its local state under `MOTYGA_HOME/app-server-daemon/`:
