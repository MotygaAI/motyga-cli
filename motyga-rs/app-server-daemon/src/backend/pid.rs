...
impl PidBackend {
    pub(crate) fn new(motyga_bin: PathBuf, pid_file: PathBuf, remote_control_enabled: bool) -> Self {
    ...
    pub(crate) fn new_update_loop(motyga_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            motyga_bin,
            pid_file,
    ...
    pub(crate) async fn start(&self) -> Result<Option<u32>> {
        ...
        let mut command = Command::new(&self.motyga_bin);
