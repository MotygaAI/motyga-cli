...
#[cfg(unix)]
use crate::managed_install::resolved_managed_motyga_bin;
...
async fn update_once(
    ...
    let managed_motyga_bin = resolved_managed_motyga_bin(&daemon.managed_motyga_bin).await?;
    let managed_identity = executable_identity(&managed_motyga_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_motyga_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_motyga_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_motyga_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Motyga binary {}",
            managed_motyga_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://motyga.com/install.sh")
        .await
        .context("failed to fetch standalone Motyga updater")?
        .error_for_status()
        .context("standalone Motyga updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone Motyga updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Motyga updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Motyga updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Motyga updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Motyga updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Motyga updater exited with status {status}")
    }
}
