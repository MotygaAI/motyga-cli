...
use motyga_rollout::remove_thread_name_entries;
...
pub(super) async fn delete_thread(
    ...
    remove_thread_name_entries(store.config.motyga_home.as_path(), thread_id)
        .await
...
fn delete_rollout_file(
    ...
    let plain_path = motyga_rollout::plain_rollout_path(rollout_path);
...
fn delete_rollout_path(
    ...
    let canonical_rollout_path = scoped_rollout_path(
        store.config.motyga_home.join(SESSIONS_SUBDIR),
        rollout_path,
        "sessions",
    )
    .or_else(|_| {
        scoped_rollout_path(
            store.config.motyga_home.join(ARCHIVED_SESSIONS_SUBDIR),
            rollout_path,
            "archived sessions",
        )
    })
