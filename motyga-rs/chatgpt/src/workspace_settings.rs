...
const MOTYGA_PLUGINS_BETA_SETTING: &str = "enable_plugins";
...
struct CachedWorkspaceSettings {
    ...
    expires_at: Instant,
    motyga_plugins_enabled: bool,
}

impl WorkspaceSettingsCache {
    fn get_motyga_plugins_enabled(&self, key: &WorkspaceSettingsCacheKey) -> Option<bool> {
        {
            ...
            if let Some(cached) = entry.as_ref()
                ...
                && cached.key == *key
            {
                return Some(cached.motyga_plugins_enabled);
            }
        }
    ...
    fn set_motyga_plugins_enabled(&self, key: WorkspaceSettingsCacheKey, enabled: bool) {
        let mut entry = match self.entry.write() {
            Ok(entry) => entry,
            Err(err) => err.into_inner(),
        };
        *entry = Some(CachedWorkspaceSettings {
            ...
            motyga_plugins_enabled: enabled,
        });
    }
}

pub async fn motyga_plugins_enabled_for_workspace(
    config: &Config,
    ...
    if let Some(cache) = cache
        && let Some(enabled) = cache.get_motyga_plugins_enabled(&cache_key)
    {
        return Ok(enabled);
    }
    ...
    let motyga_plugins_enabled = settings
        .beta_settings
        .get(MOTYGA_PLUGINS_BETA_SETTING)
        .copied()
        .unwrap_or(true);

    if let Some(cache) = cache {
        cache.set_motyga_plugins_enabled(cache_key, motyga_plugins_enabled);
    }

    Ok(motyga_plugins_enabled)
}
