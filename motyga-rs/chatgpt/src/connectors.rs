...
use motyga_connectors::DirectoryListResponse;
use motyga_connectors::merge::merge_connectors;
use motyga_connectors::merge::merge_plugin_connectors;
use motyga_core::config::Config;
pub use motyga_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use motyga_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use motyga_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use motyga_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use motyga_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use motyga_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use motyga_core::connectors::with_app_enabled_state;
...
async fn connector_auth(config: &Config) -> anyhow::Result<MotygaAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_motyga_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        ...
        "ChatGPT connectors require Motyga backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = motyga_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = motyga_connectors::list_all_connectors_with_options(
        cache_context,
...
mod tests {
    use super::*;
    use motyga_connectors::metadata::connector_install_url;
    use motyga_plugin::AppConnectorId;
