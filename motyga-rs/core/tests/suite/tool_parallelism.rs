...
async fn run_turn(test: &TestMotyga, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestMotyga, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_motyga_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestMotyga> {
    let mut builder = test_motyga().with_model("test-gpt-5.1-codex");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_motyga_with_test_tool(&server).await?;
