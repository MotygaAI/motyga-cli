...
use motyga_core::SESSIONS_SUBDIR;
...
async fn read_config_toml(motyga_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(motyga_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(motyga_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(motyga_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = motyga_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(motyga_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = motyga_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
