...
async fn motyga_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    motyga_returns_json_result("gpt-5.4".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn motyga_returns_json_result_for_gpt5_codex() -> anyhow::Result<()> {
    motyga_returns_json_result("gpt-5.4".to_string()).await
}

async fn motyga_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("motyga_output_schema".into()))
    ...
    let TestMotyga { motyga, config, .. } = test_motyga().build(&server).await?;
    ...
    let message = wait_for_event(&motyga, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
