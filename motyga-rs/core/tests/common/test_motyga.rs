...
use motyga_core::shell::Shell;
use motyga_core::shell::get_shell_by_model_provided_path;
...
use motyga_extension_api::LoadUserInstructionsFuture;
...
use motyga_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
pub fn local(cwd: AbsolutePathBuf) -> TurnEnvironmentSelection {
    TurnEnvironmentSelection {
        environment_id: motyga_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
impl TestEnv {
    pub async fn local() -> Result<Self> {
        ...
        let environment =
            motyga_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)?;
    ...
    pub fn environment(&self) -> &motyga_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                motyga_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
            ...
            let selection = TurnEnvironmentSelection {
                environment_id: motyga_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: cwd_uri.clone(),
            };
...
impl TestMotygaBuilder {
    ...
    pub fn with_auth(mut self, auth: MotygaAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestMotyga> {
    ...
    ///
    /// With no remote test configuration, or with `MOTYGA_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `MOTYGA_TEST_ENVIRONMENT=docker` or
    /// `MOTYGA_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `MOTYGA_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `MOTYGA_TEST_REMOTE_ENV` Docker-container
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let motyga_linux_sandbox_exe = Some(
            crate::find_motyga_linux_sandbox_exe()
                .context("should find binary for motyga-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let motyga_linux_sandbox_exe = None;
        let local_runtime_paths = motyga_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            motyga_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            motyga_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            motyga_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<motyga_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestMotyga> {
        ...
        let thread_manager = ThreadManager::new(
            &config,
            motyga_core::test_support::auth_manager_from_auth(auth.clone()),
            SessionSource::Exec,
            ...
            thread_store,
            motyga_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    motyga_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    motyga_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
        ...
        Ok(TestMotyga {
            ...
            cwd,
            config,
            motyga: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = motyga_utils_cargo_bin::cargo_bin("motyga") {
            config.motyga_self_exe = Some(path);
        } else if let Ok(path) = motyga_utils_cargo_bin::cargo_bin("motyga-exec") {
            // `motyga-exec` also supports `--motyga-run-as-apply-patch`, so use it
        ...
        {
            let motyga = bin_dir.join("motyga");
            let motyga_exec = bin_dir.join("motyga-exec");
            if motyga.is_file() {
                config.motyga_self_exe = Some(motyga);
            } else if motyga_exec.is_file() {
                config.motyga_self_exe = Some(motyga_exec);
            }
        }
...
pub struct TestMotyga {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub motyga: Arc<MotygaThread>,
...
impl TestMotyga {
    ...
    pub fn motyga_home_path(&self) -> &Path {
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.motyga, |event| match event {
...
pub struct TestMotygaHarness {
    server: MockServer,
    test: TestMotyga,
}

impl TestMotygaHarness {
    pub async fn new() -> Result<Self> {
        Self::with_builder(test_motyga()).await
    }

    pub async fn with_config(mutator: impl FnOnce(&mut Config) + Send + 'static) -> Result<Self> {
        Self::with_builder(test_motyga().with_config(mutator)).await
    }

    pub async fn with_builder(mut builder: TestMotygaBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestMotygaBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_motyga() -> TestMotygaBuilder {
    TestMotygaBuilder {
        ...
        auth: MotygaAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
