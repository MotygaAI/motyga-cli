...
pub use motyga_connectors::AppBranding;
pub use motyga_connectors::AppInfo;
pub use motyga_connectors::AppMetadata;
use motyga_connectors::ConnectorDirectoryCacheContext;
use motyga_connectors::ConnectorDirectoryCacheKey;
use motyga_connectors::app_is_enabled;
use motyga_connectors::apps_config_from_layer_stack;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = motyga_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        motyga_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    mcp_servers.retain(|name, _| name == MOTYGA_APPS_MCP_SERVER_NAME);
    ...
    let mcp_connection_manager = McpConnectionManager::new(
        ...
        mcp_manager.motyga_apps_tools_cache(),
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_motyga_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{MOTYGA_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let motyga_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(MOTYGA_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(MOTYGA_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(MOTYGA_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if motyga_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let chatgpt_user_id = auth.and_then(MotygaAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(MotygaAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + motyga_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    motyga_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != MOTYGA_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(motyga_connectors::accessible::AccessibleConnectorTool {
    ...
    motyga_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer(
    ...
    let app_reviewer = if server_name == MOTYGA_APPS_MCP_SERVER_NAME {
