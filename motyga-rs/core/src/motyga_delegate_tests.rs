...
use motyga_protocol::protocol::GuardianAssessmentAction;
...
use motyga_protocol::request_user_input::RequestUserInputEvent;
...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let cancel = CancellationToken::new();
    let forward = tokio::spawn(forward_ops(Arc::clone(&motyga), rx_ops, cancel));

    let submission = Submission {
        id: "sub-1".to_string(),
        op: Op::Interrupt,
        client_user_message_id: None,
        trace: Some(motyga_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
#[tokio::test]
async fn run_motyga_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    assert!(matches!(result, Err(MotygaErr::TurnAborted)));
}
