...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_motyga_version_mismatch: bool,
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_motyga_version_mismatch
        && expected_lock.motyga_version != actual_lock.motyga_version
    {
        return Err(config_lock_error(format!(
            "config lock Motyga version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_motyga_version_mismatch=true to ignore this",
            expected_lock.motyga_version, actual_lock.motyga_version
        )));
    }
