    ...
    async fn handle_call(
        ...
        match payload {
                ...
                if tool_name.namespace.is_none() && tool_name.name.as_str() == WAIT_TOOL_NAME =>
            {
                ...
                let cell_id = motyga_code_mode::CellId::new(args.cell_id);
                let wait_response = if args.terminate {
                    ...
                    exec.session
                        .services
                        .code_mode_service
                        .wait(motyga_code_mode::WaitRequest {
                            cell_id,
                ...
                if let motyga_code_mode::WaitOutcome::LiveCell(response) = &wait_response
                    && !matches!(response, motyga_code_mode::RuntimeResponse::Yielded { .. })
                {
                    ...
                    let runtime_cell_id = match response {
                        motyga_code_mode::RuntimeResponse::Yielded { cell_id, .. }
                        | motyga_code_mode::RuntimeResponse::Terminated { cell_id, .. }
                        | motyga_code_mode::RuntimeResponse::Result { cell_id, .. } => cell_id,
                    };
                    exec.session
                        .services
