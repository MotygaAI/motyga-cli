...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, MotygaErr>> {
        ...
        async move {
            match future.await {
                ...
                Err(FunctionCallError::Fatal(message)) => Err(MotygaErr::Fatal(message)),
    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: motyga_protocol::models::FunctionCallOutputPayload {
                    body: motyga_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
...
mod tests {
    ...
    impl motyga_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl motyga_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: motyga_extension_api::ToolFinishInput<'a>,
        ) -> motyga_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let mut builder =
            motyga_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
        ...
        let tool_name = motyga_tools::ToolName::plain("test_tool");
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = motyga_tools::ToolName::plain("cleanup_tool");
