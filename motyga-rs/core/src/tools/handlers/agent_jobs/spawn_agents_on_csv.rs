...
pub async fn handle(
    ...
    for (idx, row) in rows.into_iter().enumerate() {
        ...
        items.push(motyga_state::AgentJobItemCreateParams {
            item_id,
    ...
    let _job = db
        .create_agent_job(
            &motyga_state::AgentJobCreateParams {
                id: job_id.clone(),
                name: job_name,
    ...
    let failed_item_errors = if progress.failed_items > 0 {
        let items = db
            .list_agent_job_items(
                job_id.as_str(),
                Some(motyga_state::AgentJobItemStatus::Failed),
                Some(5),
            )
            .await
            .unwrap_or_default();
...
fn single_local_environment_cwd(turn: &TurnContext) -> Result<AbsolutePathBuf, FunctionCallError> {
    ...
    turn_environment.cwd().to_abs_path().map_err(|err| {
        FunctionCallError::RespondToModel(format!(
            "spawn_agents_on_csv cwd `{}` is not native to the Motyga host: {err}",
