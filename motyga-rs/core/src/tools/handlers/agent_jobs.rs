...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<motyga_state::StateRuntime>, FunctionCallError> {
...
async fn run_agent_job_loop(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    db: Arc<motyga_state::StateRuntime>,
    job_id: String,
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(motyga_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
            for item in pending_items {
                    ...
                    .await
                {
                    ...
                    Err(MotygaErr::AgentLimitReached { .. }) => {
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(motyga_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, motyga_state::AgentJobItemStatus::Running) {
...
fn build_worker_prompt(
    job: &motyga_state::AgentJob,
    item: &motyga_state::AgentJobItem,
...
fn render_instruction_template(instruction: &str, row_json: &Value) -> String {
    const OPEN_BRACE_SENTINEL: &str = "__MOTYGA_OPEN_BRACE__";
    const CLOSE_BRACE_SENTINEL: &str = "__MOTYGA_CLOSE_BRACE__";
...
fn job_runtime_timeout(job: &motyga_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &motyga_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &motyga_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
...
fn render_job_csv(
    headers: &[String],
    items: &[motyga_state::AgentJobItem],
