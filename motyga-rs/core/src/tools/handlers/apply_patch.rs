...
use motyga_apply_patch::Hunk;
use motyga_apply_patch::StreamingPatchParser;
...
use motyga_protocol::protocol::PatchApplyUpdatedEvent;
...
fn format_update_chunks_for_progress(chunks: &[motyga_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &motyga_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    motyga_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match motyga_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match motyga_apply_patch::verify_apply_patch_args(
            args,
            turn_environment.cwd(),
            fs.as_ref(),
            Some(&sandbox),
        ...
        .await
        {
            motyga_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            motyga_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            motyga_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            motyga_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match motyga_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        motyga_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
        ...
        motyga_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
