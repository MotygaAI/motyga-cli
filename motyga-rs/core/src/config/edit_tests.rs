...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(motyga_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = motyga_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let motyga_home = tmp.path();
    let link_a = motyga_home.join("a.toml");
    let link_b = motyga_home.join("b.toml");
    let config_path = motyga_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(motyga_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(motyga_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(motyga_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !motyga_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(motyga_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(motyga_home.join(CONFIG_TOML_FILE)).expect("read config");
