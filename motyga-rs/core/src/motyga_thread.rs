...
use motyga_thread_store::ThreadStoreResult;
...
/// Explains why `MotygaThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
pub struct MotygaThreadSettingsOverrides {
...
pub struct MotygaThread {
    pub(crate) motyga: Motyga,
...
/// (formerly called a conversation) in Motyga.
impl MotygaThread {
    pub(crate) fn new(
        motyga: Motyga,
        session_configured: SessionConfiguredEvent,
    ...
    pub async fn submit(&self, op: Op) -> MotygaResult<String> {
        self.motyga.submit(op).await
    }

    /// Returns the session telemetry handle for thread-scoped production instrumentation.
    pub fn session_telemetry(&self) -> SessionTelemetry {
        self.motyga.session.services.session_telemetry.clone()
    }

    pub async fn shutdown_and_wait(&self) -> MotygaResult<()> {
        self.motyga.shutdown_and_wait().await
    }

    /// Wait until the underlying session loop has terminated.
    pub async fn wait_until_terminated(&self) {
        self.motyga.session_loop_termination.clone().await;
    }
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(motyga_extension_api::ThreadResumeInput {
                    session_store: &self.motyga.session.services.session_extension_data,
                    thread_store: &self.motyga.session.services.thread_extension_data,
                })
                .await;
        }
    }
    ...
    pub async fn ensure_rollout_materialized(&self) {
        self.motyga.session.ensure_rollout_materialized().await;
    }

    #[doc(hidden)]
    pub async fn flush_rollout(&self) -> std::io::Result<()> {
        self.motyga.session.flush_rollout().await
    }
    ...
    pub async fn submit_with_trace(
        ...
        self.motyga.submit_with_trace(op, trace).await
    }
    ...
    pub async fn set_thread_memory_mode(&self, mode: ThreadMemoryMode) -> anyhow::Result<()> {
        self.motyga.set_thread_memory_mode(mode).await
    }
    ...
    /// callers that only hold a `MotygaThread`.
    /// It returns the unchanged items when this thread has no active turn.
    pub async fn inject_if_running(
        &self,
        items: Vec<ResponseItem>,
    ) -> Result<(), Vec<ResponseItem>> {
        self.motyga.session.inject_if_running(items).await
    }
    ...
    pub async fn try_start_turn_if_idle(
        ...
        self.motyga.session.try_start_turn_if_idle(items).await
    }
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: MotygaThreadSettingsOverrides,
    ) -> ConstraintResult<ThreadConfigSnapshot> {
        let updates = self.thread_settings_update(overrides).await;
        self.motyga.session.preview_settings(&updates).await
    }
    ...
    async fn thread_settings_update(
        ...
        let MotygaThreadSettingsOverrides {
            environments,
            workspace_roots,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> MotygaResult<()> {
        self.motyga.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> MotygaResult<Event> {
        self.motyga.next_event().await
    }

    pub async fn agent_status(&self) -> AgentStatus {
        self.motyga.agent_status().await
    }

    pub async fn list_background_terminals(&self) -> Vec<BackgroundTerminalInfo> {
        self.motyga.session.list_background_terminals().await
    }
    ...
    pub(crate) fn subscribe_status(&self) -> watch::Receiver<AgentStatus> {
        self.motyga.agent_status.clone()
    }
    ...
    pub async fn token_usage_info(&self) -> Option<TokenUsageInfo> {
        self.motyga.session.token_usage_info().await
    }
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> MotygaResult<()> {
        if items.is_empty() {
        ...
        let turn_context = self.motyga.session.new_default_turn().await;
        if self.motyga.session.reference_context_item().await.is_none() {
    ...
    pub async fn config(&self) -> Arc<crate::config::Config> {
        self.motyga.session.get_config().await
    }

    /// Resolves the MCP runtime configuration using this thread's extension data.
    pub async fn runtime_mcp_config(&self, config: &crate::config::Config) -> motyga_mcp::McpConfig {
        self.motyga.session.runtime_mcp_config(config).await
    }
    ...
    pub fn multi_agent_version(&self) -> Option<MultiAgentVersion> {
        self.motyga.session.multi_agent_version()
    }
    ...
    pub async fn refresh_runtime_config(&self, next_config: crate::config::Config) {
        self.motyga.session.refresh_runtime_config(next_config).await;
    }

    pub async fn environment_selections(&self) -> Vec<TurnEnvironmentSelection> {
        self.motyga.thread_environment_selections().await
    }
    ...
    pub fn enabled(&self, feature: Feature) -> bool {
        self.motyga.enabled(feature)
    }

    pub async fn increment_out_of_band_elicitation_count(&self) -> MotygaResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        let was_zero = *guard == 0;
        *guard = guard.checked_add(1).ok_or_else(|| {
            MotygaErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> MotygaResult<u64> {
        let mut guard = self.out_of_band_elicitation_count.lock().await;
        if *guard == 0 {
            return Err(MotygaErr::InvalidRequest(
