...
fn map_rollout_io_error(io_err: &std::io::Error, codex_home: &Path) -> Option<CodexErr> {
    ...
    let hint = match io_err.kind() {
        ...
        ErrorKind::AlreadyExists => format!(
            "Session storage path {} is blocked by an existing file. Remove or rename it so Motyga can create sessions.",
            sessions_dir.display()
        ),
        ...
        ErrorKind::IsADirectory | ErrorKind::NotADirectory => format!(
            "Session storage path {} has an unexpected type. Ensure it is a directory Motyga can use for session files.",
            sessions_dir.display()
        ),
        _ => return None,
    };
