...
pub(crate) fn write_curated_plugin_sha(motyga_home: &Path) {
    write_curated_plugin_sha_with(motyga_home, TEST_CURATED_PLUGIN_SHA);
}

pub(crate) fn write_curated_plugin_sha_with(motyga_home: &Path, sha: &str) {
    write_file(&motyga_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) fn write_plugins_feature_config(motyga_home: &Path) {
    write_file(
...
pub(crate) async fn load_plugins_config(motyga_home: &Path) -> crate::config::Config {
