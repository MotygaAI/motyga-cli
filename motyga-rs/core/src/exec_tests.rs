...
fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"MOTYGA_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
    assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
}
...
fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"MOTYGA_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
...
async fn exec_full_buffer_capture_ignores_expiration() -> Result<()> {
    ...
    let output = exec(
        ExecParams {
            command,
            cwd: motyga_utils_absolute_path::AbsolutePathBuf::current_dir()?,
            expiration: 1.into(),
...
async fn process_exec_tool_call_preserves_full_buffer_capture_policy() -> Result<()> {
    ...
    let cwd = motyga_utils_absolute_path::AbsolutePathBuf::current_dir()?;
...
fn windows_restricted_token_rejects_managed_root_write_profiles() {
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Special {
                value: motyga_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: motyga_protocol::permissions::FileSystemAccessMode::Write,
        },
    ]);
    let permission_profile = PermissionProfile::from_runtime_permissions(
        &file_system_policy,
...
fn windows_elevated_allows_split_restricted_read_policies() {
    ...
    let docs = motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Path { path: docs },
            access: motyga_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
    let permission_profile = PermissionProfile::from_runtime_permissions(
        &file_system_policy,
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Special {
                value: motyga_protocol::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: motyga_protocol::permissions::FileSystemAccessMode::Write,
        ...
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Path {
                path: motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: motyga_protocol::permissions::FileSystemAccessMode::Read,
        },
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Path { path: docs.clone() },
            access: motyga_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
    let permission_profile = PermissionProfile::from_runtime_permissions(
    ...
    // `.motyga`, so the restricted-token overlay only needs the extra read-only
    // docs carveout.
...
fn windows_restricted_token_rejects_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Path { path: blocked },
...
fn windows_elevated_supports_split_write_read_carveouts() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            additional_deny_read_paths: vec![],
            additional_deny_write_paths: vec![
                motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Path {
                path: motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&blocked)
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                    expected_blocked.clone(),
                )
                .expect("absolute blocked"),
            ],
            additional_deny_write_paths: vec![
                motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_blocked)
                    .expect("absolute blocked"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_globs() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(secret)
...
fn windows_elevated_rejects_reopened_writable_descendants() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        motyga_protocol::permissions::FileSystemSandboxEntry {
            path: motyga_protocol::permissions::FileSystemPath::Path {
                path: motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&nested)
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = motyga_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
