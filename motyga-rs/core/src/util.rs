...
/// `motyga_feedback::MotygaFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// motyga_core::feedback_tags!(model = "gpt-5", cached = true);
/// motyga_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
