...
use motyga_config::ConfigEditsBuilder;
use motyga_config::McpServerConfig;
use motyga_config::McpServerTransportConfig;
use motyga_config::load_global_mcp_servers;
use motyga_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(motyga_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&motyga_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&motyga_home)
