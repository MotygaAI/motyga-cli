...
impl AgentControl {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> MotygaResult<String> {
        let state = self.upgrade()?;
        let result = if let Ok(thread) = state.get_thread(agent_id).await {
            thread.motyga.session.ensure_rollout_materialized().await;
            thread.motyga.session.flush_rollout().await?;
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> MotygaResult<String> {
        let state = self.upgrade()?;
        ...
        match state.get_thread(agent_id).await {
                ...
                if !thread.config_snapshot().await.ephemeral
                    ...
                    && let Err(err) = agent_graph_store
                        .set_thread_spawn_edge_status(
                            agent_id,
                            motyga_agent_graph_store::ThreadSpawnEdgeStatus::Closed,
                        )
                        .await
                {
            ...
            Err(MotygaErr::ThreadNotFound(_)) => {}
            Err(err) => {
                warn!("failed to inspect agent before close {agent_id}: {err}");
            }
        }
        match Box::pin(self.shutdown_agent_tree(agent_id)).await {
            Err(MotygaErr::ThreadNotFound(_)) | Err(MotygaErr::InternalAgentDied) if known_agent => {
                Ok(String::new())
            }
            result => result,
        }
    ...
    pub(crate) async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> MotygaResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(MotygaErr::ThreadNotFound(_)) | Err(MotygaErr::InternalAgentDied) => {}
