...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.motyga_linux_sandbox_exe = Some(PathBuf::from("/tmp/motyga-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/motyga-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::High));
    assert_eq!(
        config.motyga_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/motyga-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/motyga-execve-wrapper"))
    );
}
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&motyga_exec_server::LOCAL_FS)),
        )
        .await;
