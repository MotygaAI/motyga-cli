...
This crate implements the business logic for Motyga. It is designed to be used by the various Motyga UIs written in Rust.
...
Note that `motyga-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.motyga` read-only.
...
Expects the binary containing `motyga-core` to run the equivalent of `motyga sandbox` when `arg0` is `motyga-linux-sandbox`. See the `motyga-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `motyga-resources/bwrap`
binary shipped with Motyga and Motyga surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Motyga also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Motyga rejects sandboxed shell commands that would enter the
...
Expects the binary containing `motyga-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--motyga-run-as-apply-patch`. See the
`motyga-arg0` crate for details.
