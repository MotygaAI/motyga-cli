...
use motyga_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == motyga_features::Feature::Artifact {
            continue;
        }
        if feature.id == motyga_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<motyga_features::FeatureToml<
                    motyga_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == motyga_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<motyga_features::FeatureToml<
                    motyga_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == motyga_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<motyga_features::FeatureToml<
                    motyga_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == motyga_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<motyga_features::FeatureToml<
                    motyga_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == motyga_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<motyga_features::FeatureToml<
                    motyga_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == motyga_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == motyga_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<motyga_features::FeatureToml<
                    motyga_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
