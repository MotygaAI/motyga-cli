...
fn write_user_skill(motyga_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = motyga_home.path().join("skills").join(dir);
...
fn user_config_layer(motyga_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(motyga_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(motyga_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(motyga_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(motyga_home, user_config_toml),
            ConfigLayerEntry::new(
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let stale_system_skill_dir = motyga_home.path().join("skills/.system/stale-skill");
    ...
    let _skills_service = SkillsService::new(
        motyga_home.path().abs(),
        /*bundled_skills_enabled*/ false,
    );

    assert!(
        !motyga_home.path().join("skills/.system").exists(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&motyga_home, "");
    let skills_service = SkillsService::new(
        motyga_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

    write_user_skill(&motyga_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&motyga_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_service, &cwd, &config_layer_stack, &[]).await;
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_motyga = cwd.path().join(".motyga");
    fs::create_dir_all(&repo_dot_motyga).expect("create repo config dir");

    write_user_skill(&motyga_home, "user", "user-skill", "from local user root");
    let repo_skill_dir = repo_dot_motyga.join("skills/repo");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&motyga_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_motyga_folder: repo_dot_motyga.abs(),
                },
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = motyga_home.path().join("skills/.system/bundled-skill");
    ...
    let config_layer_stack = config_stack(&motyga_home, "[skills.bundled]\nenabled = false\n");
    let skills_service = SkillsService::new(
        motyga_home.path().abs(),
        /*bundled_skills_enabled*/ false,
    );
...
async fn skills_for_cwd_uses_cached_result_until_force_reload() {
    ...
    write_user_skill(&motyga_home, "late", "late-skill", "added after cache");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = motyga_home.path().join("skills").join("demo");
    fs::create_dir_all(&skill_dir).expect("create skill dir");
    let skill_path = skill_dir.join("SKILL.md");
    fs::write(
        &skill_path,
    ...
    let parent_stack = config_stack(&motyga_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&motyga_home, &disabled_skill_config, &enabled_skill_config);
    let skills_service = SkillsService::new(
        motyga_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );
