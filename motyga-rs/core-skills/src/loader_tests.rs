...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".motyga";
...
async fn make_config(motyga_home: &TempDir) -> TestConfig {
    make_config_for_cwd(motyga_home, motyga_home.path().to_path_buf()).await
}
...
fn project_layers_for_cwd(cwd: &Path) -> Vec<ConfigLayerEntry> {
    ...
    layers
        .into_iter()
        .filter_map(|dir| {
            let dot_motyga = dir.join(REPO_ROOT_CONFIG_DIR_NAME);
            dot_motyga.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_motyga_folder: dot_motyga.abs(),
                    },
...
async fn make_config_for_cwd(motyga_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = motyga_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = motyga_home.path().join("etc/motyga/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/motyga");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("motyga");
...
async fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    fs::create_dir_all(&dot_motyga)?;

    let user_file = user_folder.join("config.toml").abs();
    let project_dot_motyga = dot_motyga.abs();
    ...
    assert_eq!(
        got,
        vec![
            (SkillScope::Repo, dot_motyga.join("skills")),
            (SkillScope::User, user_folder.join("skills")),
            (
                SkillScope::User,
                home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
...
fn write_skill(motyga_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&motyga_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(motyga_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &motyga_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&motyga_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    let cfg = make_config(&motyga_home).await;
...
async fn loads_skill_interface_metadata_from_yaml() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&motyga_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&motyga_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&motyga_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&motyga_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::Motyga, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(motyga_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &motyga_home.path().join("skills/shared"));
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $MOTYGA_HOME/skills/cycle/loop -> $MOTYGA_HOME/skills/cycle
    let cycle_dir = motyga_home.path().join("skills/cycle");
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&motyga_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg).await;
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = motyga_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = motyga_home.path().join("skills");
    let outcome = load_skills_from_roots(
        [SkillRoot {
...
async fn loads_valid_skill() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&motyga_home, "demo", "demo-skill", "does things\ncarefully");
    let cfg = make_config(&motyga_home).await;
...
async fn skips_hidden_and_invalid() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = motyga_home.path().join("skills/.hidden");
    ...
    let invalid_dir = motyga_home.path().join("skills/invalid");
...
async fn preserves_overlong_descriptions() {
    ...
    write_skill(&motyga_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&motyga_home).await;
    ...
    write_skill(&motyga_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert!(
        outcome.errors.is_empty(),
        "unexpected errors: {:?}",
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_motyga_dir() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_motyga_dirs_under_project_root() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&motyga_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_motyga_dir_when_not_git_repo() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&motyga_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&motyga_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_motyga_dirs() {
    let motyga_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&motyga_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&motyga_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&motyga_home, "system", "system-skill", "from system");
