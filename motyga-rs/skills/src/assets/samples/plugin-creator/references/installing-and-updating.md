...
- the user wants Motyga to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single Motyga cachebuster suffix:
...
outside Motyga depends on a specific token:
...
```bash
motyga plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `motyga plugin marketplace add` for that
path, and `motyga plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
motyga plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Motyga picks up new skills
   and tools.
...
```text
<base-version>+motyga.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+motyga.local-20260519-184516`
- `0.1.0+motyga.old-token` → `0.1.0+motyga.local-20260519-184516`
- `1.2.3-beta.1+motyga.prev` → `1.2.3-beta.1+motyga.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+motyga.local-20260519-184516`

Replace the existing Motyga cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `motyga plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `motyga plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Motyga.
...
- Use `motyga plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `motyga plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `motyga plugin add <plugin-name>@<marketplace-name>`.
