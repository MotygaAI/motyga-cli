...
pub enum Command {
    /// Submit a new Motyga Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Motyga Cloud task.
    Status(StatusCommand),
    /// List Motyga Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Motyga Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Motyga Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Motyga Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `motyga cloud` to browse).
    ...
    /// Git branch to run in Motyga Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Motyga Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Motyga Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Motyga Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
