...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_motyga_home() -> Result<()>
{
    let motyga_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        motyga_home.path(),
    ...
    let relative_path = rollout_path.strip_prefix(motyga_home.path())?.to_path_buf();
