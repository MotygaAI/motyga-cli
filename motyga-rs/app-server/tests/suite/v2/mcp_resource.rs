...
use motyga_app_server_protocol::McpResourceContent;
...
const TEST_RESOURCE_URI: &str = "test://motyga/resource";
const TEST_BLOB_RESOURCE_URI: &str = "test://motyga/resource.bin";
...
async fn orchestrator_skill_can_read_referenced_resource_without_an_executor() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "warnings": ["Orchestrator skill discovery stopped after 2 resource pages: failed to list orchestrator skill resources: resources/list failed for `motyga_apps`: Mcp error: -32603: simulated later-page failure"],
        })
    );
...
async fn disabled_orchestrator_skills_do_not_expose_skills_namespace() -> Result<()> {
    ...
    let (apps_server_url, apps_server_calls, apps_server_handle) =
        start_resource_apps_mcp_server().await?;
    let responses_server_uri = responses_server.uri();
    let (_motyga_home, mut mcp) = start_resource_test_app_server_with_extra_config(
        &apps_server_url,
...
-> Result<(String, Arc<ResourceAppsMcpCalls>, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/motyga/ps/mcp", mcp_service);
