...
use motyga_app_server::PluginStartupTasks;
use motyga_app_server::RemoteControlStartupMode;
use motyga_app_server::run_main_with_transport_options;
use motyga_app_server_protocol::JSONRPCError;
use motyga_app_server_protocol::JSONRPCResponse;
use motyga_app_server_protocol::RemoteControlClient;
use motyga_app_server_protocol::RemoteControlClientsListOrder;
use motyga_app_server_protocol::RemoteControlClientsListParams;
use motyga_app_server_protocol::RemoteControlClientsListResponse;
use motyga_app_server_protocol::RemoteControlClientsRevokeParams;
use motyga_app_server_protocol::RemoteControlClientsRevokeResponse;
use motyga_app_server_protocol::RemoteControlConnectionStatus;
use motyga_app_server_protocol::RemoteControlDisableResponse;
use motyga_app_server_protocol::RemoteControlEnableResponse;
...
use motyga_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = motyga_home.path().join("managed_config.toml");
    let socket_path = motyga_home.path().join("app-server.sock");
    let transport =
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let motyga_home = TempDir::new()?;
    let listener = configured_remote_control_listener(motyga_home.path()).await?;
    let websocket_url = format!(
        "ws://{}/backend-api/wham/remote/control/server",
        listener.local_addr()?
    );
    let state_db =
        StateRuntime::init(motyga_home.path().to_path_buf(), "test-provider".to_string()).await?;
    state_db
    ...
    let _app_server = TestAppServer::new_with_args(motyga_home.path(), &["--listen", "off"]).await?;
...
async fn listen_off_ignores_persisted_enable_when_disabled_by_requirements() -> Result<()> {
    ...
    let mut app_server =
        TestAppServer::new_with_args(motyga_home.path(), &["--listen", "off"]).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let motyga_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(motyga_home.path()).await?;
    let mut mcp = TestAppServer::new(motyga_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let motyga_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(motyga_home.path()).await?;
    let mut mcp = TestAppServer::new(motyga_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let motyga_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(motyga_home.path()).await?;
    let mut mcp = TestAppServer::new(motyga_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let motyga_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(motyga_home.path()).await?;
    let mut mcp = TestAppServer::new(motyga_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl ClientManagementRemoteControlBackend {
    async fn start(motyga_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(motyga_home).await?;
...
async fn configured_remote_control_listener(motyga_home: &std::path::Path) -> Result<TcpListener> {
