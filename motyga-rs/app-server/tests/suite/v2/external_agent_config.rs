...
fn external_agent_home(motyga_home: &Path) -> PathBuf {
    motyga_home.join(concat!(".", "cl", "aude"))
}
...
-> Result<()> {
    ...
    let home_dir = motyga_home.path().display().to_string();
    let sqlite_home_dir = sqlite_home.path().display().to_string();
    let mut mcp = TestAppServer::new_with_env(
        motyga_home.path(),
        &[
            ("HOME", Some(home_dir.as_str())),
            ("MOTYGA_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ],
    )
    .await?;
    ...
    let state_db =
        motyga_state::StateRuntime::init(sqlite_home.path().to_path_buf(), "mock_provider".into())
            .await?;
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    let source_home = external_agent_home(motyga_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(
        source_home.join("settings.json"),
        r#"{"env":{"FOO":"bar"}}"#,
    )?;
    std::fs::write(motyga_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = motyga_home.path().display().to_string();
    let analytics_capture_file = motyga_home.path().join("analytics-events.jsonl");
    ...
    let mut mcp = TestAppServer::new_with_env(
        ...
        &[
            ("HOME", Some(home_dir.as_str())),
            (
                "MOTYGA_ANALYTICS_EVENTS_CAPTURE_FILE",
    ...
    let events = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if captured_events.iter().any(|event| {
                event["event_type"] == "motyga_onboarding_external_agent_import_complete"
    ...
    let event = events
        .iter()
        .find(|event| {
            event["event_type"] == "motyga_onboarding_external_agent_import_failure"
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let motyga_home = TempDir::new()?;
    write_analytics_config(motyga_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        motyga_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let missing_session_path =
        external_agent_home(motyga_home.path()).join("projects/repo/missing.jsonl");
    let project_root = motyga_home.path().join("repo");
    let home_dir = motyga_home.path().display().to_string();
    let mut mcp =
        TestAppServer::new_with_env(motyga_home.path(), &[("HOME", Some(home_dir.as_str()))])
            .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "motyga_onboarding_external_agent_import_complete",
    )
    .await?;
    let event_params = &event["event_params"];
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "motyga_onboarding_external_agent_import_failure",
    )
    .await?;
    let event_params = &event["event_params"];
    assert_eq!(event_params["import_id"], serde_json::json!(import_id));
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(motyga_home.path()).join("projects/repo");
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let motyga_home = TempDir::new()?;
    create_config_toml(motyga_home.path(), &server.uri())?;
    let mut config = std::fs::read_to_string(motyga_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
"#,
    ...
    std::fs::write(motyga_home.path().join("config.toml"), config)?;
    let project_root = motyga_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = external_agent_home(motyga_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
...
fn write_analytics_config(motyga_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        motyga_home.join("config.toml"),
