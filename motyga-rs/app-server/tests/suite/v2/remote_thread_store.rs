...
//! the temporary `motyga_home` must not contain rollout session files or sqlite
...
use motyga_app_server::in_process::InProcessClientHandle;
use motyga_app_server::in_process::InProcessServerEvent;
...
use motyga_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under motyga_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(motyga_home.path(), &server.uri(), &store_id)?;

    let thread_store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };

    let mut client = start_in_process_server(motyga_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(motyga_home.path())?;
...
async fn start_in_process_server(motyga_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .motyga_home(motyga_home.to_path_buf())
            .fallback_cwd(Some(motyga_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(motyga_home: &Path) -> Result<()> {
    ...
    let mut entries = motyga_home_entries(motyga_home)?;
    // Bazel test runs may initialize shell snapshot storage under motyga_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in motyga_home"
    );

    Ok(())
}

fn motyga_home_entries(motyga_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(motyga_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
