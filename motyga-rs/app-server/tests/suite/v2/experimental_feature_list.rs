...
use motyga_app_server_protocol::ExperimentalFeature;
...
use motyga_app_server_protocol::ExperimentalFeatureStage;
...
-> Result<()> {
    let motyga_home = TempDir::new()?;
    let project_cwd = motyga_home.path().join("project");
