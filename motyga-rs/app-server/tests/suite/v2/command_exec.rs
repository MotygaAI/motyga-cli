...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$MOTYGA_HOME\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", motyga_home.path().display()),
            stderr: String::new(),
        }
    );
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${MOTYGA_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let motyga_home = TempDir::new()?;
    create_config_toml(motyga_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(motyga_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::new(motyga_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let motyga_home = TempDir::new()?;
    let command_dir = motyga_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !motyga_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(motyga_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "motyga-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(motyga_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(motyga_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(motyga_home: &Path, inserted_config: &str) -> Result<()> {
    let config_path = motyga_home.join("config.toml");
