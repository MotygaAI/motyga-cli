...
async fn thread_delete_deletes_spawned_descendants() -> Result<()> {
    ...
    let parent_id = create_delete_test_rollout(motyga_home.path(), /*minute*/ 0, "parent")?;
    let child_id = create_delete_test_rollout(motyga_home.path(), /*minute*/ 1, "child")?;
    let grandchild_id =
        create_delete_test_rollout(motyga_home.path(), /*minute*/ 2, "grandchild")?;
...
fn create_delete_test_rollout(motyga_home: &Path, minute: u8, preview: &str) -> Result<String> {
    create_fake_rollout(
        motyga_home,
