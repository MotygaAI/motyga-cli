...
async fn turn_steer_requires_active_turn() -> Result<()> {
    ...
    mount_analytics_capture(&server, &motyga_home).await?;

    let mut mcp = TestAppServer::new_without_managed_config(&motyga_home).await?;
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "motyga_turn_steer_event").await?;
