...
fn command_hook_hash(
    ...
    let Ok(value) = motyga_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    motyga_config::version_for_toml(&value)
}

fn write_user_hook_config(motyga_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(motyga_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = motyga_home.join("plugins/cache/test/demo/local");
...
fn write_project_hook_config(dot_motyga_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_motyga_folder)?;
    std::fs::write(
        dot_motyga_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(motyga_home.path())?;
...
async fn hooks_list_warms_plugin_capabilities_for_thread_start() -> Result<()> {
  ...
  "hooks": {
    ...
    let plugin_mcp_path = motyga_home
        .path()
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let motyga_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(motyga_home.path(), "{ not-json")?;
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".motyga"))?;
    std::fs::write(
        workspace.path().join(".motyga/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".motyga/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".motyga"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".motyga"), "echo worktree hook")?;
    set_project_trust_level(motyga_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".motyga/config.toml"))?;
    ...
    .await??;
    let _: motyga_app_server_protocol::ConfigWriteResponse = to_response(response)?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let motyga_home = TempDir::new()?;
    let hook_script_path = motyga_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = motyga_home.path().join("user_prompt_submit_hook_log.jsonl");
    std::fs::write(
        &hook_script_path,
        format!(
            r#"import json
