...
async fn turn_interrupt_aborts_running_turn() -> Result<()> {
    ...
    let server =
        ...
        .await;
    create_config_toml(&motyga_home, &server.uri(), "never", "workspace-write")?;
...
async fn turn_interrupt_rejects_completed_turn() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_auto_env(&motyga_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn turn_interrupt_resolves_pending_command_approval_request() -> Result<()> {
    ...
    .await;
    create_config_toml(&motyga_home, &server.uri(), "untrusted", "read-only")?;
