...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "motyga@example.com"])?;
    run_git(root, &["config", "user.name", "Motyga Tests"])?;
...
fn disable_plugin_startup_tasks(motyga_home: &Path) -> Result<()> {
...
fn marketplace_install_root(motyga_home: &Path) -> std::path::PathBuf {
    motyga_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(motyga_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&motyga_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(motyga_home.path())?;

    let mut mcp = TestAppServer::new(motyga_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    let debug_root = expected_installed_root(motyga_home.path(), "debug")?;
    let tools_root = expected_installed_root(motyga_home.path(), "tools")?;
