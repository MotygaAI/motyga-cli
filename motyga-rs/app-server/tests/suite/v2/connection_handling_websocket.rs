...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let motyga_home = TempDir::new()?;
    let token_file = motyga_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(motyga_home.path(), "ws://0.0.0.0:0", &auth_args).await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let motyga_home = TempDir::new()?;
    let shared_secret_file = motyga_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "motyga-enroller".to_string(),
        "--ws-audience".to_string(),
        "motyga-app-server".to_string(),
        "--ws-max-clock-skew-seconds".to_string(),
        "1".to_string(),
    ];

    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(motyga_home.path(), "ws://127.0.0.1:0", &auth_args).await?;
...
async fn websocket_transport_rejects_unauthenticated_non_loopback_startup() -> Result<()> {
    ...
    let output =
        run_websocket_server_to_completion_with_args(motyga_home.path(), "ws://0.0.0.0:0", &[])
            .await?;
    assert!(
        !output.status.success(),
...
pub(super) async fn spawn_websocket_server(motyga_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(motyga_home, "ws://127.0.0.1:0", &[]).await
}
