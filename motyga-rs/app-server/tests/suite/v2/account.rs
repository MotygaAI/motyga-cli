...
use motyga_login::CLIENT_ID_OVERRIDE_ENV_VAR;
use motyga_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
use motyga_login::login_with_api_key;
use motyga_login::login_with_bedrock_api_key;
...
const LOGIN_ISSUER_ENV_VAR: &str = "MOTYGA_APP_SERVER_LOGIN_ISSUER";
...
fn create_config_toml(motyga_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(motyga_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(motyga_home.path().join("auth.json").exists());
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(motyga_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<motyga_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(motyga_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![motyga_app_server_protocol::UserInput::Text {
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/motyga/device"));
