...
use motyga_cloud_config::cloud_config_bundle_loader;
...
/// Shared app-server entry point for loading effective Motyga configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn motyga_home(&self) -> &Path {
        self.motyga_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.motyga_home())
    }
    ...
    pub(crate) fn replace_cloud_config_bundle_loader(
        ...
        let loader =
            cloud_config_bundle_loader(auth_manager, chatgpt_base_url, self.motyga_home.clone());
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(motyga_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        let mut config = Config::load_default_with_cli_overrides_for_motyga_home(
        ...
        {
            let user_config_path = self.loader_overrides.user_config_path(self.motyga_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = motyga_core::config::ConfigBuilder::default()
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.motyga_self_exe = self.arg0_paths.motyga_self_exe.clone();
        config.motyga_linux_sandbox_exe = self.arg0_paths.motyga_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(motyga_home: PathBuf) -> Self {
        Self::new_for_tests(
            motyga_home,
            Vec::new(),
