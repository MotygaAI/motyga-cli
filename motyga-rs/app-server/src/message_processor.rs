...
use motyga_app_server_protocol::ChatgptAuthTokensRefreshParams;
use motyga_app_server_protocol::ChatgptAuthTokensRefreshReason;
use motyga_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use motyga_app_server_protocol::experimental_required_message;
...
use motyga_login::auth::ExternalAuth;
use motyga_login::auth::ExternalAuthRefreshContext;
use motyga_login::auth::ExternalAuthRefreshReason;
use motyga_login::auth::ExternalAuthTokens;
use motyga_protocol::ThreadId;
use motyga_protocol::auth::AuthMode as LoginAuthMode;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: MotygaFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = motyga_core::thread_store_from_config(config.as_ref(), state_db.clone());
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let motyga_request = deserialize_client_request(&request);
                let result = match motyga_request {
                    Ok(motyga_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = motyga_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = motyga_request.experimental_reason()
        ...
        let serialization_scope = motyga_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: motyga_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match motyga_request {
