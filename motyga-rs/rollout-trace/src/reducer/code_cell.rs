...
//! - runtime work starts as soon as Motyga dispatches the tool.
//!
...
pub(super) struct PendingCodeCellStart {
    ...
    pub(super) motyga_turn_id: Option<String>,
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        let Some(motyga_turn_id) = motyga_turn_id else {
            bail!(
                "code cell start {} did not include a Motyga turn id",
                started.code_cell_id
            );
        };
        self.validate_code_cell_turn(&thread_id, &motyga_turn_id)?;
    ...
    pub(super) fn terminate_running_code_cells_for_turn_end(
        ...
        let code_cell_ids: Vec<_> = self
            ...
            .values()
            .filter(|cell| {
                cell.motyga_turn_id == motyga_turn_id
    ...
    pub(super) fn code_cell_event_thread_id(
        ...
        self.rollout
            ...
            .with_context(|| {
                format!(
                    "{event_name} {runtime_cell_id} referenced unknown Motyga turn {motyga_turn_id}"
                )
            })
    }
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, motyga_turn_id: &str) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("code cell start referenced unknown thread {thread_id}");
        }
        let Some(turn) = self.rollout.motyga_turns.get(motyga_turn_id) else {
            bail!("code cell start referenced unknown Motyga turn {motyga_turn_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but Motyga turn {motyga_turn_id} belongs \
                 to {}",
                turn.thread_id
            );
        }
