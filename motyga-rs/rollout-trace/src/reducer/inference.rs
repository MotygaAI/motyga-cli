...
impl TraceReducer {
    ...
    /// If a call is still `Running`, Motyga stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.motyga_turn_id == motyga_turn_id
