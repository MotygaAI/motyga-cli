...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, motyga_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, motyga_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_motyga_turn_id: motyga_turn_id,
    ...
    fn tool_thread_id(
        ...
        self.rollout
            ...
            .with_context(|| {
                format!("tool call start referenced unknown Motyga turn {motyga_turn_id}")
            })
    }

    fn validate_tool_turn(&self, thread_id: &str, motyga_turn_id: Option<&str>) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("tool call start referenced unknown thread {thread_id}");
        }
        if let Some(motyga_turn_id) = motyga_turn_id {
            let Some(turn) = self.rollout.motyga_turns.get(motyga_turn_id) else {
                bail!("tool call start referenced unknown Motyga turn {motyga_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Motyga turn {motyga_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
