...
use crate::protocol_event::motyga_turn_trace_event;
...
pub const MOTYGA_ROLLOUT_TRACE_ROOT_ENV: &str = "MOTYGA_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `MOTYGA_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Motyga
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(MOTYGA_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Motyga turn lifecycle events from protocol lifecycle events.
    pub fn record_motyga_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            motyga_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Motyga did while executing that boundary.
    pub fn record_tool_call_event(&self, motyga_turn_id: impl Into<MotygaTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(motyga_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_motyga_turn_id: impl Into<MotygaTurnId>,
    ...
    pub fn record_motyga_turn_started(&self, motyga_turn_id: impl Into<MotygaTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let motyga_turn_id = motyga_turn_id.into();
        context.append_with_context_best_effort(
            motyga_turn_id.clone(),
            RawTraceEventPayload::MotygaTurnStarted {
                motyga_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(motyga_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one Motyga turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        ...
        let event_context = RawTraceEventContext {
            ...
            motyga_turn_id: Some(motyga_turn_id),
        };
