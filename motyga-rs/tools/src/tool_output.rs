...
impl ToolOutput for motyga_protocol::mcp::CallToolResult {
    fn log_preview(&self) -> String {
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    motyga_protocol::models::ContentItem::InputText { text }
                    | motyga_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    motyga_protocol::models::ContentItem::InputImage { image_url, detail } => {
