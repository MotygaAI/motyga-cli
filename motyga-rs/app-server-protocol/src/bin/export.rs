...
#[derive(Parser, Debug)]
#[command(
    about = "Generate TypeScript bindings and JSON Schemas for the Motyga app-server protocol"
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    motyga_app_server_protocol::generate_ts_with_options(
        ...
        motyga_app_server_protocol::GenerateTsOptions {
            experimental_api: args.experimental,
            ..motyga_app_server_protocol::GenerateTsOptions::default()
        },
    )?;
    motyga_app_server_protocol::generate_json_with_experimental(&args.out_dir, args.experimental)
}
