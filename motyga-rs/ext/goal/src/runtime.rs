...
struct GoalRuntimeInner {
    thread_id: ThreadId,
    state_dbs: Arc<motyga_state::StateRuntime>,
...
pub struct PreviousGoalSnapshot {
    pub goal_id: String,
    pub status: motyga_state::ThreadGoalStatus,
    pub objective: String,
}

impl From<&motyga_state::ThreadGoal> for PreviousGoalSnapshot {
    fn from(goal: &motyga_state::ThreadGoal) -> Self {
        Self {
...
impl GoalRuntimeHandle {
    ...
    pub async fn apply_external_goal_set(
        ...
        match goal.status {
            ...
            motyga_state::ThreadGoalStatus::BudgetLimited => {
                if self.inner.accounting_state.current_turn_id().is_none() {
                    self.inner.accounting_state.clear_active_goal();
                }
            }
            motyga_state::ThreadGoalStatus::Paused
            | motyga_state::ThreadGoalStatus::Blocked
            | motyga_state::ThreadGoalStatus::UsageLimited
            | motyga_state::ThreadGoalStatus::Complete => {
                self.inner.accounting_state.clear_active_goal();
            }
        }
        Ok(())
    ...
    pub(crate) async fn stop_active_goal_for_turn(
        ...
        let (event_name, status) = match reason {
            ActiveGoalStopReason::TurnError => {
                ("turn-error", motyga_state::ThreadGoalStatus::Blocked)
            }
            ActiveGoalStopReason::UsageLimit => {
                ("usage-limit", motyga_state::ThreadGoalStatus::UsageLimited)
            }
        };
        self.account_active_goal_progress(
            turn_id,
        ...
        let can_stop = active_goal.status == motyga_state::ThreadGoalStatus::Active
            || (active_goal.status == motyga_state::ThreadGoalStatus::BudgetLimited
                && status == motyga_state::ThreadGoalStatus::UsageLimited);
        if !can_stop {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    pub async fn restore_after_resume(&self) -> Result<(), String> {
        ...
        let goal = self
            ...
            .map_err(|err| err.to_string())?;
        match goal {
            Some(goal) if goal.status == motyga_state::ThreadGoalStatus::Active => {
                self.inner
                    .accounting_state
    ...
    pub(crate) async fn continue_if_idle(&self) -> Result<(), String> {
        ...
        if goal.status != motyga_state::ThreadGoalStatus::Active {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    async fn account_idle_goal_progress(
        ...
        Ok(match outcome {
            ...
            motyga_state::GoalAccountingOutcome::Unchanged(_) => {
    ...
    ) -> Result<Option<motyga_state::ThreadGoalStatus>, String> {
        let goal = self
            .inner
            .state_dbs
            .thread_goals()
