...
/// Contributes Motyga memory read-path prompt context and memory read tools.
...
pub(crate) struct MemoriesExtensionConfig {
    ...
    pub(crate) motyga_home: AbsolutePathBuf,
}
...
impl ContextContributor for MemoriesExtension {
    fn contribute_thread_context<'a>(
        ...
        Box::pin(async move {
            ...
            build_memory_tool_developer_instructions(&config.motyga_home)
                .await
...
impl ToolContributor for MemoriesExtension {
    fn tools(
        ...
        tools::memory_tools(
            LocalMemoriesBackend::from_motyga_home(&config.motyga_home),
