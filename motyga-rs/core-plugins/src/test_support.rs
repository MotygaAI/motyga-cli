...
pub(crate) async fn load_plugins_config(motyga_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let motyga_home = AbsolutePathBuf::try_from(motyga_home).expect("motyga home should be absolute");
