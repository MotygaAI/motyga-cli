...
use motyga_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use motyga_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
const CURATED_PLUGINS_FETCH_REF: &str = "refs/motyga/curated-sync";
...
// Keep this comfortably above a normal sync attempt so we do not race another Motyga process.
...
pub fn curated_plugins_repo_path(motyga_home: &Path) -> PathBuf {
    motyga_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn curated_plugins_api_marketplace_path(motyga_home: &Path) -> PathBuf {
    curated_plugins_repo_path(motyga_home).join(".agents/plugins/api_marketplace.json")
}

pub fn read_curated_plugins_sha(motyga_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(motyga_home).as_path())
}

fn curated_plugins_sha_path(motyga_home: &Path) -> PathBuf {
    motyga_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_openai_plugins_repo(motyga_home: &Path) -> Result<String, String> {
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    let _file_guard = lock_curated_plugins_startup_sync(motyga_home)?;

    match sync_openai_plugins_repo_via_git(motyga_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_openai_plugins_repo_via_http(motyga_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(motyga_home) {
...
fn lock_curated_plugins_startup_sync(motyga_home: &Path) -> Result<File, String> {
    let lock_path = motyga_home.join(CURATED_PLUGINS_SYNC_LOCK_FILE);
...
fn sync_openai_plugins_repo_via_backup_archive(
    ...
    let repo_path = curated_plugins_repo_path(motyga_home);
    let sha_path = curated_plugins_sha_path(motyga_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(motyga_home: &Path) -> bool {
    curated_plugins_repo_path(motyga_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && motyga_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
