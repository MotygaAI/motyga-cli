...
async fn save_remote_plugin_share_creates_workspace_plugin() {
    ...
    assert_eq!(
        local_paths::load_plugin_share_local_paths(motyga_home.path()).unwrap(),
...
async fn list_remote_plugin_shares_fetches_created_workspace_plugins() {
    ...
    let local_plugin_path =
        AbsolutePathBuf::try_from(motyga_home.path().join("local-plugin")).unwrap();
    ...
    let result = list_remote_plugin_shares(&config, Some(&auth), motyga_home.path())
        .await
        .unwrap();
...
async fn delete_remote_plugin_share_deletes_workspace_plugin() {
    ...
    delete_remote_plugin_share(&config, Some(&auth), motyga_home.path(), "plugins_123")
        .await
        .unwrap();
    assert_eq!(
        local_paths::load_plugin_share_local_paths(motyga_home.path()).unwrap(),
