...
fn record_duration_records_histogram() -> Result<()> {
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "motyga.request_latency");
    assert!(!bounds.is_empty());
    assert_eq!(bucket_counts.iter().sum::<u64>(), 1);
    assert_eq!(sum, 15.0);
    assert_eq!(count, 1);
    let metric = crate::harness::find_metric(&resource_metrics, "motyga.request_latency")
        .expect("motyga.request_latency metric should exist");
...
fn record_duration_seconds_uses_fractional_seconds_and_scaled_buckets() -> Result<()> {
    ...
    for duration in [
        ...
        metrics.record_duration_seconds_with_description(
            "motyga.request_duration_seconds",
            "Duration of Motyga requests in seconds.",
            duration,
            &[("method", "initialize")],
        )?;
    }
    ...
    let resource_metrics = latest_metrics(&exporter);
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "motyga.request_duration_seconds");
    assert_eq!(
        bounds,
        vec![
    ...
    assert_eq!(count, 3);
    let metric = crate::harness::find_metric(&resource_metrics, "motyga.request_duration_seconds")
        .expect("motyga.request_duration_seconds metric should exist");
...
fn timer_result_records_success() -> Result<()> {
    ...
    {
        let timer = metrics.start_timer("motyga.request_latency", &[("route", "chat")]);
        assert!(timer.is_ok());
    }
    ...
    let attrs = attributes_to_map(
        crate::harness::find_metric(&resource_metrics, "motyga.request_latency")
            ...
            .expect("motyga.request_latency attributes should exist"),
    );
