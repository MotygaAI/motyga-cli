//! Aggregates MCP server connections for Motyga.
//!
...
use crate::motyga_apps_cache::MotygaAppsToolsCacheKey;
...
use motyga_config::McpServerAuth;
...
use motyga_protocol::protocol::McpStartupCompleteEvent;
use motyga_protocol::protocol::McpStartupFailure;
use motyga_protocol::protocol::McpStartupFailureReason;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        motyga_apps_tools_cache_key: MotygaAppsToolsCacheKey,
        prefix_mcp_tool_names: bool,
        ...
        let chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_motyga_backend())
            .map(motyga_model_provider::auth_provider_from_auth);
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // For built-in Motyga Apps, `MOTYGA_CONNECTORS_TOKEN` is a debug
            ...
            let shares_motyga_apps_tools_cache =
                should_share_motyga_apps_tools_cache(&server_name, uses_env_bearer_token);
            let motyga_apps_tools_cache_context = shares_motyga_apps_tools_cache.then(|| {
                motyga_apps_tools_cache
                    .context(motyga_home.clone(), motyga_apps_tools_cache_key.clone())
            });
            // If Motyga Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient MotygaAuth provider.
            let runtime_auth_provider =
                if server_name == MOTYGA_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
    ...
    pub fn is_host_owned_motyga_apps_server(&self, server_name: &str) -> bool {
        server_name == MOTYGA_APPS_MCP_SERVER_NAME && self.server_metadata.contains_key(server_name)
    }
    ...
    /// Force-refresh motyga apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_motyga_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(MOTYGA_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{MOTYGA_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        let fetch_ticket = managed_client
            .motyga_apps_tools_cache_context
            .as_ref()
            .map(|cache_context| cache_context.begin_fetch(MotygaAppsToolsFetchSource::HardRefresh));
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{MOTYGA_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        let tools =
            match (
                managed_client.motyga_apps_tools_cache_context.as_ref(),
                fetch_ticket,
            ) {
...
fn should_share_motyga_apps_tools_cache(server_name: &str, uses_env_bearer_token: bool) -> bool {
    server_name == MOTYGA_APPS_MCP_SERVER_NAME && !uses_env_bearer_token
}
