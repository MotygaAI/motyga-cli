...
fn test_mcp_config(motyga_home: PathBuf) -> McpConfig {
    McpConfig {
        chatgpt_base_url: "https://api.motyga.com".to_string(),
        ...
        connector_snapshot: motyga_connectors::ConnectorSnapshot::default(),
    }
}
...
fn tool_plugin_provenance_collects_app_and_mcp_sources() {
    ...
    catalog.register(McpServerRegistration::from_plugin(
        ...
        /*plugin_order*/ 0,
        motyga_apps_mcp_server_config("https://alpha.example", /*apps_mcp_product_sku*/ None),
    ));
...
fn selected_mcp_attribution_does_not_join_an_unrelated_local_summary() {
    ...
    catalog.register(McpServerRegistration::from_selected_plugin(
        ...
        /*selection_order*/ 0,
        motyga_apps_mcp_server_config("https://github.example", /*apps_mcp_product_sku*/ None),
    ));
    config.mcp_server_catalog = catalog.build();
    config.connector_snapshot =
        motyga_connectors::ConnectorSnapshot::from_plugin_capability_summaries(&[
...
#[test]
fn motyga_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        motyga_apps_mcp_url_for_base_url("https://api.motyga.com/backend-api"),
        "https://api.motyga.com/backend-api/wham/apps"
    );
    assert_eq!(
        motyga_apps_mcp_url_for_base_url("https://api.motyga.com"),
        "https://api.motyga.com/backend-api/wham/apps"
    );
    assert_eq!(
        motyga_apps_mcp_url_for_base_url("http://localhost:8080/api/motyga"),
        "http://localhost:8080/api/motyga/apps"
    );
    assert_eq!(
        motyga_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/motyga/apps"
    );
}

#[test]
fn motyga_apps_server_config_uses_legacy_motyga_apps_path() {
    let config =
        motyga_apps_mcp_server_config("https://api.motyga.com", /*apps_mcp_product_sku*/ None);
    let url = match &config.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for motyga apps"),
    };
...
#[test]
fn motyga_apps_server_config_forwards_configured_product_sku_header() {
    let config = motyga_apps_mcp_server_config("https://api.motyga.com", Some("tpp"));
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(motyga_home.path().to_path_buf());
    ...
    let motyga_apps = effective
        .get(MOTYGA_APPS_MCP_SERVER_NAME)
        .expect("motyga apps server should exist");
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let motyga_apps = motyga_apps
        .configured_config()
        .expect("motyga apps should use configured transport");
