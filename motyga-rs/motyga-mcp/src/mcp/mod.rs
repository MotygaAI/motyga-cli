...
use motyga_model_provider::CHATGPT_MOTYGA_BASE_URL;
...
use crate::motyga_apps_cache::motyga_apps_tools_cache_key;
use crate::connection_manager::McpConnectionManager;
use crate::runtime::McpRuntimeContext;
use crate::server::EffectiveMcpServer;

pub const MOTYGA_APPS_MCP_SERVER_NAME: &str = "motyga_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const MOTYGA_CONNECTORS_TOKEN_ENV_VAR: &str = "MOTYGA_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `motyga_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `motyga-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// Motyga home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `motyga-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_motyga_apps_enabled(config: &McpConfig, auth: Option<&MotygaAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(MotygaAuth::uses_motyga_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let chatgpt_origin = url::Url::parse(CHATGPT_MOTYGA_BASE_URL)
        .ok()
        .map(|url| url.origin());
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_motyga_apps_enabled(config, auth) {
        servers.remove(MOTYGA_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
fn motyga_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(MOTYGA_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(MOTYGA_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(MOTYGA_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_motyga_apps_base_url(base_url: &str) -> String {
...
fn motyga_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_motyga_apps_base_url(base_url);
    let (base_url, default_path) = if base_url.contains("/backend-api") {
        (base_url, "wham/apps")
    } else if base_url.contains("/api/motyga") {
        (base_url, "apps")
    } else {
        (format!("{base_url}/api/motyga"), "apps")
    };
    format!("{base_url}/{default_path}")
}

pub fn motyga_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        motyga_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    let base_url = normalize_motyga_apps_base_url(chatgpt_base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/motyga") {
        base_url
    } else {
        format!("{base_url}/api/motyga")
    };
    mcp_server_config_for_url(
...
fn mcp_server_config_for_url(
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: motyga_apps_mcp_bearer_token_env_var(),
            http_headers,
