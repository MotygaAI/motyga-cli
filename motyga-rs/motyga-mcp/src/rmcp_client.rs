...
//! and exposing cached Motyga Apps tools while a client is still connecting.
...
use crate::motyga_apps::normalize_motyga_apps_callable_name;
use crate::motyga_apps::normalize_motyga_apps_callable_namespace;
use crate::motyga_apps::normalize_motyga_apps_tool_title;
use crate::motyga_apps_cache::MotygaAppsToolsCacheContext;
use crate::motyga_apps_cache::MotygaAppsToolsFetchSource;
use crate::motyga_apps_cache::load_startup_cached_motyga_apps_server_info;
...
use motyga_protocol::protocol::McpStartupStatus;
use motyga_protocol::protocol::McpStartupUpdateEvent;
...
use motyga_rmcp_client::StdioServerLauncher;
use motyga_rmcp_client::ToolWithConnectorId;
...
/// MCP server capability indicating that Motyga should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "motyga/sandbox-state-meta";
pub const OPENAI_FORM_CAPABILITY: &str = "openai/form";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "motyga.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "motyga.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const MOTYGA_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const MOTYGA_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) motyga_apps_tools_cache_context: Option<MotygaAppsToolsCacheContext>,
}
...
impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .motyga_apps_tools_cache_context
            .as_ref()
            .and_then(MotygaAppsToolsCacheContext::current_tools)
        {
            emit_duration(
        ...
        if self.motyga_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct MotygaAppsStartupReconnectState {
...
#[derive(Clone)]
struct MotygaAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct MotygaAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<MotygaAppsStartupReconnectState>,
    startup_status_context: Option<MotygaAppsStartupStatusContext>,
}

impl MotygaAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(MotygaAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = Some(MotygaAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = motyga_apps_reconnect_backoff(state.consecutive_failures);
...
fn motyga_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    MOTYGA_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(MOTYGA_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    motyga_apps_tools_cache_context: Option<MotygaAppsToolsCacheContext>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_motyga_apps_mcp_server = server_name == MOTYGA_APPS_MCP_SERVER_NAME;
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_motyga_apps_mcp_server: bool,
    pub(crate) cached_server_info: Option<McpServerInfo>,
    pub(crate) motyga_apps_tools_cache_context: Option<MotygaAppsToolsCacheContext>,
    pub(crate) tool_filter: ToolFilter,
    pub(crate) startup_complete: Arc<AtomicBool>,
    pub(crate) startup_reconnect: Option<Arc<MotygaAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let tool_filter = server
            ...
            .map(ToolFilter::from_config)
            .unwrap_or_default();
        let cached_server_info = if is_motyga_apps_mcp_server {
            motyga_apps_tools_cache_context
                .as_ref()
                .and_then(load_startup_cached_motyga_apps_server_info)
        } else {
            None
        };
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            motyga_apps_tools_cache_context: motyga_apps_tools_cache_context.clone(),
            runtime_context,
            runtime_auth_provider,
            client_elicitation_capability,
            supports_openai_form_elicitation,
        ...
        let startup_reconnect = is_motyga_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                MotygaAppsStartupReconnect::new(Arc::new(move || startup.start()))
        ...
        if motyga_apps_tools_cache_context
            .as_ref()
            .is_some_and(MotygaAppsToolsCacheContext::has_current_tools)
        {
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.motyga_apps_tools_cache_context
            .as_ref()
            .is_some_and(MotygaAppsToolsCacheContext::has_current_tools)
    }
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
        ...
        Some(if self.is_motyga_apps_mcp_server {
            prepare_motyga_apps_tools_for_model(tools, &self.tool_plugin_provenance)
        } else {
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_motyga_apps_mcp_server: bool,
...
/// Presents declared Motyga Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
fn prepare_motyga_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_motyga_apps_mcp_server {
        motyga_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Motyga Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn motyga_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_motyga_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_motyga_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_motyga_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = motyga_apps_tools_cache_context
        .as_ref()
        .map(|cache_context| cache_context.begin_fetch(MotygaAppsToolsFetchSource::Startup));
    ...
    let tools = match (motyga_apps_tools_cache_context.as_ref(), fetch_ticket) {
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("motyga-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Motyga"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(motyga_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    #[test]
    fn motyga_apps_connector_metadata_is_preserved() {
        ...
        let tool_info = tool_info_from_listed_tool(
            ...
            /*is_motyga_apps_mcp_server*/ true,
        ...
        let expected = ToolInfo {
            server_name: MOTYGA_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "motyga_apps__gmail".to_string(),
