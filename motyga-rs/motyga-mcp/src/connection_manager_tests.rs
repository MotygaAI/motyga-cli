use super::*;
use crate::motyga_apps_cache::MotygaAppsToolsCache;
...
use crate::rmcp_client::MOTYGA_APPS_RECONNECT_INITIAL_BACKOFF;
use crate::rmcp_client::MotygaAppsStartupReconnect;
...
use motyga_rmcp_client::InProcessTransportFactory;
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        motyga_apps_tools_cache_context: None,
    }
}
...
async fn create_ready_async_managed_client(tools: Vec<ToolInfo>) -> AsyncManagedClient {
    AsyncManagedClient {
        ...
        .boxed()
        .shared(),
        is_motyga_apps_mcp_server: false,
...
fn create_test_manager_with_failed_apps_startup(
    ...
    manager.clients.insert(
        ...
        AsyncManagedClient {
            ...
            cached_server_info: None,
            motyga_apps_tools_cache_context: Some(cache_context),
            tool_filter: ToolFilter::default(),
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: Some(Arc::new(MotygaAppsStartupReconnect::new(reconnect_factory))),
            tool_plugin_provenance: Arc::new(ToolPluginProvenance::default()),
            cancel_token: CancellationToken::new(),
        },
    );
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(MOTYGA_APPS_MCP_SERVER_NAME, "upload").tool;
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        motyga_rmcp_client::Elicitation::Mcp(
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let elicitation = motyga_rmcp_client::Elicitation::Mcp(
        CreateElicitationRequestParams::FormElicitationParams {
            meta: None,
    ...
    let (
        motyga_protocol::mcp::RequestId::String(request_a_id),
        motyga_protocol::mcp::RequestId::String(request_b_id),
...
#[test]
fn motyga_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_motyga_apps_tools_cache(
...
#[tokio::test]
async fn list_all_tools_uses_shared_motyga_apps_cache_while_client_is_pending() {
    let motyga_home = tempdir().expect("tempdir");
    let cache_context = create_motyga_apps_tools_cache_context(
        motyga_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__motyga_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, MOTYGA_APPS_MCP_SERVER_NAME);
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(MOTYGA_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_motyga_apps_cache_is_empty() {
    let motyga_home = tempdir().expect("tempdir");
    let cache_context = create_motyga_apps_tools_cache_context(
        motyga_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_motyga_apps_cache_when_client_startup_fails() {
    let motyga_home = tempdir().expect("tempdir");
    let cache_context = create_motyga_apps_tools_cache_context(
        motyga_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_motyga_apps_startup_and_reuses_client() {
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(MOTYGA_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_motyga_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        MOTYGA_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::motyga_apps_mcp_server_config(
        "https://docs.example",
...
#[test]
fn host_owned_motyga_apps_requires_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    ...
    assert!(!manager.is_host_owned_motyga_apps_server(MOTYGA_APPS_MCP_SERVER_NAME));
}

#[test]
fn host_owned_motyga_apps_matches_reserved_name_with_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionManager::new_uninitialized(
        &approval_policy,
    ...
    let server = EffectiveMcpServer::configured(crate::motyga_apps_mcp_server_config(
    ...
    assert!(manager.is_host_owned_motyga_apps_server(MOTYGA_APPS_MCP_SERVER_NAME));
    assert!(!manager.is_host_owned_motyga_apps_server("docs"));
}
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = MOTYGA_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
