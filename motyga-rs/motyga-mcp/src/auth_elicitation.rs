...
//! Session orchestration stays in `motyga-core`.

use motyga_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_MOTYGA_APPS_META_KEY: &str = "_motyga_apps";
...
pub struct MotygaAppsConnectorAuthFailure {
...
pub struct MotygaAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct MotygaAppsAuthElicitationPlan {
    pub auth_failure: MotygaAppsConnectorAuthFailure,
    pub elicitation: MotygaAppsAuthElicitation,
}

#[derive(Serialize)]
struct MotygaAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<MotygaAppsConnectorAuthFailure> {
    ...
    Some(MotygaAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<MotygaAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(MotygaAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &MotygaAppsConnectorAuthFailure,
) -> MotygaAppsAuthElicitation {
    MotygaAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_MOTYGA_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: MotygaAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("motyga_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &MotygaAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Motyga.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn parses_auth_failure_from_trusted_connector_metadata() {
        assert_eq!(
            connector_auth_failure_from_tool_result(
                ...
                Some("https://motyga.com/apps/google-calendar/connector_calendar".to_string()),
            ),
            Some(MotygaAppsConnectorAuthFailure {
                connector_id: "connector_calendar".to_string(),
                connector_name: "Google Calendar".to_string(),
                install_url: "https://motyga.com/apps/google-calendar/connector_calendar"
                    .to_string(),
    ...
    fn rejects_missing_or_mismatched_connector_ids() {
        ...
        assert_eq!(
            connector_auth_failure_from_tool_result(
                ...
                Some("Google Drive"),
                Some("https://motyga.com/apps/google-drive/connector_drive".to_string()),
            ),
            None
        );
    }
        ...
        assert_eq!(
            ...
            MotygaAppsAuthElicitation {
                ...
                message: "Reconnect Google Calendar on ChatGPT to restore access for this request."
                    .to_string(),
                url: "https://motyga.com/apps/google-calendar/connector_calendar".to_string(),
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "motyga_apps_auth_call_123");
    }
}
