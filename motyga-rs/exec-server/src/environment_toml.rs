...
pub(crate) fn environment_provider_from_motyga_home(
    motyga_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = motyga_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = motyga_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
"#,
        ...
        for (index, (contents, expected)) in cases.into_iter().enumerate() {
            let path = motyga_home.path().join(format!("environments-{index}.toml"));
    ...
    #[tokio::test]
    async fn environment_provider_from_motyga_home_uses_present_environments_file() {
        let motyga_home = tempdir().expect("tempdir");
        std::fs::write(
            motyga_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider =
            environment_provider_from_motyga_home(motyga_home.path()).expect("environment provider");
    ...
    #[tokio::test]
    async fn environment_provider_from_motyga_home_falls_back_when_file_is_missing() {
        let motyga_home = tempdir().expect("tempdir");
