...
//!
//! Keeping this wrapper separate avoids depending on the full Motyga binary's
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let runtime_paths =
        ExecServerRuntimePaths::new(current_exe, /*motyga_linux_sandbox_exe*/ None)?;
    motyga_exec_server::run_main("ws://127.0.0.1:0", runtime_paths).await
}
