...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("motyga-exec-tests");
...
fn exec_default_stderr_filter_suppresses_otel_self_diagnostics() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::error!(target: "motyga_exec_test", "real exec error");
    });
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = motyga_protocol::protocol::W3cTraceContext {
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let motyga_home = tempdir().expect("create temp motyga home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: motyga_app_server_protocol::SessionSource::Exec,
        thread_source: None,
        agent_nickname: None,
        agent_role: None,
        git_info: None,
        name: None,
        turns: vec![
            motyga_app_server_protocol::Turn {
                id: "turn-1".to_string(),
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(motyga_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(motyga_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
"#,
    ...
    .expect("write config");
    let requirements_path = motyga_home.path().join("requirements.toml");
    std::fs::write(
        &requirements_path,
        r#"
...
async fn thread_start_params_include_user_thread_source() {
    ...
    assert_eq!(
        ...
        Some(motyga_app_server_protocol::ThreadSource::User)
    );
}
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(motyga_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        thread: motyga_app_server_protocol::Thread {
            ...
            thread_source: Some(motyga_app_server_protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            git_info: None,
        ...
        approval_policy: motyga_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: motyga_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: motyga_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
