...
/// While we may add an `apply-patch` subcommand to the `motyga` CLI multitool
/// at some point, we must ensure that the smaller `motyga-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(motyga_utils_cargo_bin::cargo_bin("motyga-exec")?)
        .arg(MOTYGA_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
