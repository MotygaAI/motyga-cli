...
use motyga_app_server_protocol::SkillsListEntry;
...
use motyga_core_skills::model::SkillToolDependency;
...
pub(crate) fn find_app_mentions(
    ...
    for app in apps.iter().filter(|app| is_app_mentionable(app)) {
        let slug = motyga_connectors::metadata::connector_mention_slug(app);
