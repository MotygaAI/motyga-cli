...
use motyga_app_server_protocol::NetworkAccess;
...
async fn forked_thread_history_line_without_name_shows_id_once_snapshot() {
    ...
    chat.config.motyga_home =
        motyga_utils_absolute_path::AbsolutePathBuf::from_absolute_path(temp.path())
...
async fn app_server_forked_thread_history_line_uses_app_server_title_snapshot() {
    ...
    assert!(
        ...
        "app-server fork title lookup should not read local MOTYGA_HOME"
    );
