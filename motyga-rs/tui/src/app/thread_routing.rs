...
impl App {
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                    ...
                    )
                {
                    ...
                    match &params.request {
                        motyga_app_server_protocol::McpServerElicitationRequest::Form {
                            message,
                            ..
                        ...
                        motyga_app_server_protocol::McpServerElicitationRequest::OpenAiForm {
                            ..
                        }
                        | motyga_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                ...
                                request_id.clone(),
                                motyga_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        let history_config = motyga_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().motyga_home.clone(),
            &self.chat_widget.config_ref().history,
        );
        tokio::spawn(async move {
            if let Err(err) =
                motyga_message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                motyga_message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(motyga_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),
