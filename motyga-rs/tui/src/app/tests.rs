...
use motyga_app_server_protocol::AgentMessageDeltaNotification;
...
use motyga_app_server_protocol::FileUpdateChange;
...
use motyga_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use motyga_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use motyga_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use motyga_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use motyga_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
...
use motyga_app_server_protocol::UserInput as AppServerUserInput;
...
use motyga_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let motyga_app_server_client::AppServerEvent::ServerNotification(
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::MotygaOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: motyga_app_server_protocol::CollabAgentTool::Wait,
                status: motyga_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(motyga_app_server_protocol::ItemCompletedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: motyga_app_server_protocol::CollabAgentTool::SendInput,
                status: motyga_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    motyga_app_server_protocol::CollabAgentState {
                        status: motyga_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.motyga_home = motyga_home.path().to_path_buf().abs();
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let motyga_home = tempdir()?;
        app.config.motyga_home = motyga_home.path().to_path_buf().abs();
        app.config.sqlite_home = motyga_home.path().to_path_buf();
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::MotygaOp(op)) => op,
        other => panic!("expected MotygaOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let motyga_home = tempdir()?;
    app.config.motyga_home = motyga_home.path().to_path_buf().abs();
    let config_toml_path = motyga_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            motyga_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            motyga_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            motyga_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: motyga_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                source: motyga_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            motyga_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        motyga_app_server_client::AppServerEvent::ServerNotification(
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        motyga_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let motyga_home = app_server
        .motyga_home_path(&app.chat_widget.config_ref().motyga_home)
        .expect("motyga home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&motyga_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&motyga_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{motyga_home}/../other/{suffix}"),
        format!("{motyga_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&motyga_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/motyga\\home"),
        &format!("/tmp/motyga/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/motyga\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/motyga\\/a");
    let attachments_dir = app.chat_widget.config_ref().motyga_home.join("attachments");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        motyga_error_info: Some(AppServerMotygaErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.motyga/archived_sessions/rollout.jsonl",
        )),
        thread_id,
    };
    let expected = format!(
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_motyga_backend_auth: app.chat_widget.has_motyga_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    motyga_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: motyga_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                motyga_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                motyga_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.motyga/skills/abc/SKILL.md"),
