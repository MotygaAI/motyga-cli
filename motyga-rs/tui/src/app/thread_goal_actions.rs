...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `motyga` to start a saved session, or `motyga resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let motyga_home = app_server.motyga_home_path(&self.config.motyga_home);
        match goal_files::objective_text_for_edit(app_server, motyga_home.as_ref(), &goal.objective)
            .await
        {
