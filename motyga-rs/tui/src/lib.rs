...
// The standalone `motyga-tui` binary prints a short help message before the
...
use motyga_app_server_client::RemoteAppServerClient;
use motyga_app_server_client::RemoteAppServerConnectArgs;
pub use motyga_app_server_client::RemoteAppServerEndpoint;
use motyga_app_server_protocol::Account as AppServerAccount;
...
use motyga_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
pub(crate) use motyga_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "motyga-tui.log";
...
fn remove_legacy_tui_log_file(motyga_home: &Path) {
    ...
    let _ = std::fs::remove_file(motyga_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let motyga_home = find_motyga_home().wrap_err("failed to resolve MOTYGA_HOME")?;
            motyga_app_server_client::app_server_control_socket_path(&motyga_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(motyga_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = motyga_app_server_client::app_server_control_socket_path(motyga_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_motyga_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        motyga_feedback::MotygaFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = motyga_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let motyga_home = match find_motyga_home() {
        Ok(motyga_home) => motyga_home.to_path_buf(),
        Err(err) => {
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&motyga_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&motyga_home).await
    } else {
        None
    };
    ...
    remove_legacy_tui_log_file(config.motyga_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        motyga_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(motyga_otel::OtelProvider::metrics) {
        let _ = motyga_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            motyga_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
    ...
    match effective_toml.try_into() {
        Ok(config_toml) => {
            match motyga_app_server_client::migrate_personality_if_needed(
                &config.motyga_home,
                &config_toml,
                state_db.clone(),
            )
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("motyga_core=info,motyga_tui=info,motyga_rmcp_client=info")
        });
    ...
    let feedback = motyga_feedback::MotygaFeedback::new();
...
async fn run_ratatui_app(
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.motyga_home.to_path_buf(),
                /*enable_motyga_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `motyga {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_motyga_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.motyga_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        motyga_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    fn write_session_rollout(
        ...
        let session_meta = motyga_protocol::protocol::SessionMeta {
            ...
            source: motyga_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(motyga_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: motyga_app_server_client::app_server_control_socket_path(&motyga_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://motyga.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("motyga.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(motyga_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let motyga_home = TempDir::new()?;
        let socket_path =
            motyga_app_server_client::app_server_control_socket_path(motyga_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(motyga_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            motyga_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = motyga_state::StateRuntime::init(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            motyga_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    -> color_eyre::Result<()> {
        ...
        let logs_db_path = motyga_state::logs_db_path(&sqlite_home);
        ...
        assert!(
            motyga_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let motyga_home = temp_dir.path().to_path_buf();
