...
//! local spritesheets. The TUI resolves them from the public Motyga pets CDN on
//! first use, verifies that the downloaded file has the expected spritesheet
//! geometry, and installs it into a versioned cache under MOTYGA_HOME.
//!
...
pub(crate) fn builtin_spritesheet_path(motyga_home: &Path, file: &str) -> PathBuf {
    pack_dir(motyga_home).join("assets").join(file)
}
...
pub(crate) fn ensure_builtin_pet(motyga_home: &Path, pet: catalog::BuiltinPet) -> Result<()> {
    let destination = builtin_spritesheet_path(motyga_home, pet.spritesheet_file);
...
fn pack_dir(motyga_home: &Path) -> PathBuf {
    motyga_home.join(PET_PACK_DIR).join(PET_PACK_VERSION)
}
...
#[cfg(test)]
pub(crate) fn write_test_pack(motyga_home: &Path) {
    let assets_dir = pack_dir(motyga_home).join("assets");
