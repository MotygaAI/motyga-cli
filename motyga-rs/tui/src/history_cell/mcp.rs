...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<motyga_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<motyga_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(motyga_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`motyga-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `motyga mcp add`.
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            crate::terminal_hyperlinks::osc8_hyperlink("https://motyga.com/docs", "MCP docs")
                .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, motyga_protocol::mcp::Tool>,
