...
use motyga_app_server_protocol::SandboxPolicy;
...
use motyga_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "motyga-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $MOTYGA_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Motyga app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Motyga", version, about = "Bootstrap Motyga app-server", long_about = None)]
struct Cli {
    /// Path to the `motyga` CLI binary. When set, requests use stdio by
    /// spawning `motyga app-server` as a child process.
    #[arg(long, env = "MOTYGA_BIN", global = true)]
    motyga_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--motyga-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "MOTYGA_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `motyga` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `motyga app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/motyga-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `motyga app-server --listen`.
    ...
    /// Send a user message through the Motyga app-server.
    SendMessage {
        /// User message to send to Motyga.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the Motyga app-server.
    GetAccountRateLimits,
    /// List the available models from the Motyga app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Motyga app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "MOTYGA_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let motyga_bin = motyga_bin.unwrap_or_else(|| PathBuf::from("motyga"));
            serve(&motyga_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(motyga_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(motyga_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(motyga_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --motyga-bin and does not support --url");
            }
            let motyga_bin = motyga_bin.context("plugin-analytics-smoke requires --motyga-bin")?;
            plugin_analytics_smoke::run(&motyga_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --motyga-bin and does not support --url"
                );
            }
            let motyga_bin =
                motyga_bin.context("plugin-analytics-mutation-smoke requires --motyga-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --motyga-bin and does not support --url");
            }
            let motyga_bin = motyga_bin.context("plugin-remote-uninstall requires --motyga-bin")?;
...
fn resolve_endpoint(motyga_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if motyga_bin.is_some() && url.is_some() {
        bail!("--motyga-bin and --url are mutually exclusive");
    }
    if let Some(motyga_bin) = motyga_bin {
        return Ok(Endpoint::SpawnMotyga(motyga_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if motyga_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --motyga-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(motyga_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(motyga_bin);
        if let Some(motyga_bin_parent) = motyga_bin.parent() {
            let mut path = OsString::from(motyga_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", motyga_bin.display()))?;
...
fn serve(motyga_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/motyga-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,motyga_=trace {}",
    ...
    println!("started motyga app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnMotyga(motyga_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut MotygaClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = MotygaClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = MotygaClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (motyga_bin, url) {
        (Some(_), Some(_)) => bail!("--motyga-bin and --url are mutually exclusive"),
        (Some(motyga_bin), None) => {
            let server = BackgroundAppServer::spawn(&motyga_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve motyga-app-server-test-client binary path")?;
...
impl MotygaClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnMotyga(motyga_bin) => Self::spawn_stdio(motyga_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(motyga_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(motyga_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let motyga_bin_display = motyga_bin.display();
        let mut cmd = Command::new(motyga_bin);
        if let Some(motyga_bin_parent) = motyga_bin.parent() {
            let mut path = OsString::from(motyga_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut motyga_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{motyga_bin_display}` app-server"))?;

        let stdin = motyga_app_server
            .stdin
            .take()
            .context("motyga app-server stdin unavailable")?;
        let stdout = motyga_app_server
            .stdout
            .take()
            .context("motyga app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: motyga_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `motyga-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "motyga-toy-app-server".to_string(),
                    title: Some("Motyga Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: motyga_app_server_protocol::LoginAccountParams::Chatgpt {
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: motyga_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to motyga app-server")?;
                    return Ok(());
                }
                bail!("motyga app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from motyga app-server")?;
                if bytes == 0 {
                    bail!("motyga app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = motyga_core::otel_init::build_provider(
            &config,
...
impl Drop for MotygaClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[motyga app-server exited: {status}]");
            return;
        }
