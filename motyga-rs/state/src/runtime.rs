...
impl RuntimeDbSpec {
    fn path(self, motyga_home: &Path) -> PathBuf {
        motyga_home.join(self.filename)
    }
}
...
impl StateRuntime {
    /// Initialize the state runtime using the provided Motyga home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `motyga_home`,
    /// keeping logs in a dedicated file to reduce lock contention with the
    /// rest of the state store.
    pub async fn init(motyga_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        Self::init_inner(
            motyga_home,
            default_provider,
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(motyga_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let state_path = STATE_DB.path(motyga_home.as_path());
        let logs_path = LOGS_DB.path(motyga_home.as_path());
        let goals_path = GOALS_DB.path(motyga_home.as_path());
        let memories_path = MEMORIES_DB.path(motyga_home.as_path());
    ...
    /// Return the configured Motyga home directory for this runtime.
    pub fn motyga_home(&self) -> &Path {
...
pub fn state_db_path(motyga_home: &Path) -> PathBuf {
    STATE_DB.path(motyga_home)
}
...
pub fn logs_db_path(motyga_home: &Path) -> PathBuf {
    LOGS_DB.path(motyga_home)
}
...
pub fn goals_db_path(motyga_home: &Path) -> PathBuf {
    GOALS_DB.path(motyga_home)
}
...
pub fn memories_db_path(motyga_home: &Path) -> PathBuf {
    MEMORIES_DB.path(motyga_home)
}

pub fn runtime_db_paths(motyga_home: &Path) -> Vec<RuntimeDbPath> {
    RUNTIME_DBS
        .iter()
        .map(|spec| RuntimeDbPath {
            label: spec.label,
            path: spec.path(motyga_home),
        })
        .collect()
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        let path = state_db_path(motyga_home.as_path());
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&motyga_home)
            .await
            .expect("create motyga home");
        let state_path = state_db_path(motyga_home.as_path());
        let pool = SqlitePool::connect_with(
            SqliteConnectOptions::new()
                .filename(&state_path)
                .create_if_missing(true),
