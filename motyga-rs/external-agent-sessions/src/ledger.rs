...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(motyga_home)?.contains_current_source(source_path)
}
...
pub fn record_completed_session_imports(
    ...
    let mut ledger = load_import_ledger(motyga_home)?;
    ...
    save_import_ledger(motyga_home, &ledger)
}
...
pub(super) fn load_import_ledger(
    ...
    let path = import_ledger_path(motyga_home);
...
fn import_ledger_path(motyga_home: &Path) -> PathBuf {
    motyga_home.join(SESSION_IMPORT_LEDGER_FILE)
}
