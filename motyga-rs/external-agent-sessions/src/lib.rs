...
pub fn prepare_validated_session_import(
    ...
    let has_been_imported = has_current_session_been_imported(motyga_home, &session.path)?;
...
mod tests {
    ...
    fn skips_session_that_was_already_imported() {
        ...
        let motyga_home = root.path().join("motyga-home");
        let source_path = root.path().join("session.jsonl");
        std::fs::write(&source_path, "{}\n").expect("session");
        ledger::record_imported_session(&motyga_home, &source_path, ThreadId::new())
            .expect("record import");

        let pending =
            prepare_validated_session_import(&motyga_home, session_migration(&source_path))
