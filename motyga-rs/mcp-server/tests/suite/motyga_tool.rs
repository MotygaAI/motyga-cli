...
use motyga_mcp_server::ExecApprovalElicitRequestParams;
use motyga_mcp_server::ExecApprovalResponse;
use motyga_mcp_server::PatchApprovalElicitRequestParams;
use motyga_mcp_server::PatchApprovalResponse;
use motyga_protocol::protocol::FileChange;
use motyga_protocol::protocol::ReviewDecision;
use motyga_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(MOTYGA_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "motyga" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let motyga_request_id = mcp_process
        .send_motyga_tool_call(MotygaToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            motyga_request_id.to_string(),
            params.motyga_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `motyga` tool call completes and that the file was created.
    let motyga_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(motyga_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(motyga_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    motyga_mcp_tool_call_id: String,
    motyga_event_id: String,
    thread_id: motyga_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Motyga to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let motyga_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        ...
        motyga_mcp_tool_call_id,
        motyga_event_id,
        motyga_command: command,
        motyga_cwd: workdir.to_path_buf(),
        motyga_call_id: "call1234".to_string(),
        motyga_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "motyga" tool request that will trigger the apply_patch command
    let motyga_request_id = mcp_process
        .send_motyga_tool_call(MotygaToolCallParam {
    ...
    // Verify the original `motyga` tool call completes
    let motyga_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(motyga_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_motyga_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    motyga_tool_passes_base_instructions()
        .await
        .expect("motyga tool should pass base instructions");
}

async fn motyga_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `motyga mcp` with a specific config.toml.
    let motyga_home = TempDir::new()?;
    create_config_toml(motyga_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(motyga_home.path()).await?;
    ...
    .await??;
    assert_eq!(motyga_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(motyga_response.id, RequestId::Number(motyga_request_id));
    assert_eq!(
        motyga_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": motyga_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("motyga tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Motyga to apply proposed code changes?".to_string());
...
/// Create a Motyga config that uses the mock server as the model provider.
