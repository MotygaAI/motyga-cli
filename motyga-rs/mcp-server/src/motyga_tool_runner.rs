//! Asynchronous worker that executes a **Motyga** tool-call inside a spawned
...
use motyga_core::config::Config as MotygaConfig;
...
/// To adhere to MCP `tools/call` response format, include the Motyga
...
/// Run a complete Motyga session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_motyga_tool_session(
    id: RequestId,
    ...
    // Use the original MCP request ID as the `sub_id` for the Motyga submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_motyga_uuid.lock().await.remove(&id);
        return;
    }

    run_motyga_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_motyga_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_motyga_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<MotygaThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(motyga_event_to_notification(&event)) above has
