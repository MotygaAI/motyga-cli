//! Custom CA handling for Motyga outbound HTTP and websocket clients.
//!
//! Motyga constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `MOTYGA_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Motyga itself commonly runs spawned test processes under seatbelt, so this is not just
...
pub const MOTYGA_CA_CERT_ENV: &str = "MOTYGA_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set MOTYGA_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Motyga custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `MOTYGA_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Motyga HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Motyga custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `MOTYGA_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Motyga custom
...
fn build_reqwest_client_with_env(
    ...
    // identities (e.g. Motyga's MotygaSandboxOnline) — rustls performs the handshake in-process with no
    ...
    info!(
        motyga_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `MOTYGA_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Motyga-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(MOTYGA_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: MOTYGA_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Motyga CA overrides.
    ///
    /// This accepts a few common real-world variants so Motyga behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Motyga only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_motyga_env() {
        let env = map_env(&[
            (MOTYGA_CA_CERT_ENV, "/tmp/motyga.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/motyga.pem"))
        );
    }
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(MOTYGA_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
