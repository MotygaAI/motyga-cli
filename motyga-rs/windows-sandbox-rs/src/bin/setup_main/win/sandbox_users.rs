...
use motyga_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "MotygaSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Motyga sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(motyga_home);
...
pub(super) fn prepare_setup_marker(motyga_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(motyga_home).join("setup_marker.json");
