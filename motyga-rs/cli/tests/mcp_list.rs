...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = motyga_command(motyga_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(motyga_home.path()).await?;
    ...
    let mut list_cmd = motyga_command(motyga_home.path())?;
    ...
    let mut list_json_cmd = motyga_command(motyga_home.path())?;
    ...
    let mut get_cmd = motyga_command(motyga_home.path())?;
    ...
    assert!(stdout.contains("remove: motyga mcp remove docs"));

    let mut get_json_cmd = motyga_command(motyga_home.path())?;
    get_json_cmd
