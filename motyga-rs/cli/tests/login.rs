...
use motyga_login::REVOKE_TOKEN_URL_OVERRIDE_ENV_VAR;
...
fn write_file_auth_config(motyga_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(motyga_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(motyga_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(motyga_home.path())?;
    ...
    let auth = read_auth_json(motyga_home.path())?;
...
async fn device_login_revokes_existing_auth_before_requesting_new_tokens() -> Result<()> {
    ...
    .env("no_proxy", "127.0.0.1,localhost")
    .env_remove("MOTYGA_ACCESS_TOKEN")
