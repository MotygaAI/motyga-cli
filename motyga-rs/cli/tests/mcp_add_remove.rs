...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = motyga_command(motyga_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = motyga_command(motyga_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = motyga_command(motyga_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!motyga_home.path().join(".credentials.json").exists());
    assert!(!motyga_home.path().join(".env").exists());
