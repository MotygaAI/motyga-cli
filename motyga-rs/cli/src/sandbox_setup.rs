...
pub(crate) struct SandboxSetupCommand {
    ...
    /// MOTYGA_HOME for the Motyga user. Required with --user.
    #[arg(long = "motyga-home", value_name = "DIR")]
...
async fn run_elevated(cmd: SandboxSetupCommand) -> anyhow::Result<()> {
    ...
    motyga_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.motyga_home.as_path(),
        identity.real_user.as_str(),
    )?;
    ConfigEditsBuilder::new(identity.motyga_home.as_path())
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let motyga_home = match cmd.motyga_home.clone() {
            Some(motyga_home) => motyga_home,
            None => find_motyga_home()?.to_path_buf(),
        };
    ...
    let motyga_home = cmd
        .motyga_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--motyga-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.motyga_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.motyga"))
        );
    }
    ...
    #[test]
    fn requires_motyga_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--motyga-home".to_string(),
            r"C:\Users\alice\.motyga".to_string(),
        ])
        .expect("parse")
