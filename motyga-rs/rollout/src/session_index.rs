...
pub async fn append_thread_name(
    ...
    append_session_index_entry(motyga_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(motyga_home);
...
fn session_index_path(motyga_home: &Path) -> PathBuf {
    motyga_home.join(SESSION_INDEX_FILE)
}
