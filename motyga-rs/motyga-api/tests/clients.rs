...
use motyga_api::ResponsesClient;
use motyga_api::ResponsesOptions;
use motyga_client::HttpTransport;
use motyga_client::Request;
use motyga_client::RequestBody;
use motyga_client::Response;
use motyga_client::StreamResponse;
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, motyga_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, motyga_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, motyga_client::RequestCompression::None);
    Ok(())
}
