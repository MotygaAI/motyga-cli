...
use motyga_api::RetryConfig;
use motyga_client::ReqwestTransport;
...
async fn models_client_hits_models_endpoint() {
    ...
    let base_url = format!("{}/api/motyga", server.uri());
    ...
    assert_eq!(received[0].url.path(), "/api/motyga/models");
}
