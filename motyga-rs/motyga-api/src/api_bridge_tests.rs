...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, MotygaErr::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let MotygaErr::UnexpectedStatus(err) = err else {
        panic!("expected MotygaErr::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let MotygaErr::CyberPolicy { message } = err else {
        panic!("expected MotygaErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let MotygaErr::InvalidRequest(message) = err else {
        panic!("expected MotygaErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("motyga_other"),
    );
    headers.insert(
    ...
    let MotygaErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected MotygaErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_surfaces_rate_limit_body_message() {
    ...
    let MotygaErr::RetryLimit(retry_limit) = err else {
        panic!("expected MotygaErr::RetryLimit, got {err:?}");
    };
    assert_eq!(
