...
impl<T: HttpTransport> ModelsClient<T> {
    ...
    fn append_client_version_query(req: &mut motyga_client::Request, client_version: &str) {
                ...
                let simple =
                    serde_json::from_slice::<OpenAiSimpleModelsResponse>(&resp.body).map_err(|e| {
                        ApiError::Stream(format!(
                            "failed to decode models response as motyga ({motyga_err}) or openai list ({e}); body: {}",
...
mod tests {
    ...
    async fn appends_client_version_query() {
        ...
        let client = ModelsClient::new(
            transport.clone(),
            provider("https://example.com/api/motyga"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            url,
            "https://example.com/api/motyga/models?client_version=0.99.0"
        );
    }
