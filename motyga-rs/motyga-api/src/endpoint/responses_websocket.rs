...
const X_MOTYGA_TURN_STATE_HEADER: &str = "x-codex-turn-state";
...
async fn connect_websocket(
    ...
    // If a Motyga-specific CA bundle is configured, build an explicit rustls connector so this
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_MOTYGA_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
