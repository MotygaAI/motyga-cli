...
use chrono::DateTime;
use chrono::Utc;
use motyga_protocol::auth::PlanType;
use motyga_protocol::error::MotygaErr;
use motyga_protocol::error::RetryLimitReachedError;
use motyga_protocol::error::UnexpectedResponseError;
use motyga_protocol::error::UsageLimitReachedError;
use http::HeaderMap;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> MotygaErr {
    match err {
        ApiError::ContextWindowExceeded => MotygaErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => MotygaErr::QuotaExceeded,
        ApiError::UsageNotIncluded => MotygaErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => MotygaErr::Stream(message, delay),
        ApiError::Stream(msg) => MotygaErr::Stream(msg, None),
        ApiError::ServerOverloaded => MotygaErr::ServerOverloaded,
        // Terminal on purpose: keep it OUT of MotygaErr::Stream, which the turn loop treats as retryable.
        ApiError::Gateway { message } => MotygaErr::Gateway(message),
        ApiError::Api { status, message } => {
            let user_message = api_error_user_message(status, &message);
            MotygaErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                user_message,
                url: None,
        ...
        ApiError::InvalidRequest { message } => MotygaErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => MotygaErr::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return MotygaErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        MotygaErr::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        MotygaErr::InvalidImageRequest()
                    } else {
                        MotygaErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return MotygaErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return MotygaErr::UsageNotIncluded;
                        }
                    }

                    MotygaErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => MotygaErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
                server_message: None,
            }),
            TransportError::Timeout => MotygaErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                MotygaErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => MotygaErr::Stream(msg, None),
    }
}
