...
function findMotygaExecutable() {
  ...
  const motygaExecutable = path.join(
    vendorRoot,
    targetTriple,
    "bin",
    process.platform === "win32" ? "motyga.exe" : "motyga",
  );
  if (existsSync(motygaExecutable)) {
    return motygaExecutable;
  }
...
const binaryPath = findMotygaExecutable();
...
/**
 * Use heuristics to detect the package manager that was used to install Motyga
...
const packageManagerEnvVar =
  detectPackageManager() === "bun"
    ? "MOTYGA_MANAGED_BY_BUN"
    : "MOTYGA_MANAGED_BY_NPM";
const env = {
  ...process.env,
  [packageManagerEnvVar]: "1",
  MOTYGA_MANAGED_PACKAGE_ROOT: realpathSync(path.join(__dirname, "..")),
};
