...
./scripts/stage_npm_packages.py \
  ...
  --package motyga \
  --package motyga-responses-api-proxy \
  --package motyga-sdk
```
...
When `--package motyga` is provided, the staging helper builds the lightweight
`@motyga/cli` meta package plus all platform-native `@motyga/cli` variants
