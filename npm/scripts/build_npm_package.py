...
MOTYGA_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = MOTYGA_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "motyga-rs" / "responses-api-proxy" / "npm"
MOTYGA_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"
MOTYGA_NPM_NAME = "@motyga/cli"
MOTYGA_PACKAGE_COMPONENT = "motyga-package"

# `npm_name` is the local optional-dependency alias consumed by `bin/motyga.js`.
# The underlying package published to npm is always `@motyga/cli`.
MOTYGA_PLATFORM_PACKAGES: dict[str, dict[str, str]] = {
    "motyga-linux-x64": {
    ...
    "motyga-darwin-x64": {
        "npm_name": "@motyga/cli-darwin-x64",
        "npm_tag": "darwin-x64",
    ...
    "motyga-darwin-arm64": {
        "npm_name": "@motyga/cli-darwin-arm64",
    ...
    "motyga-win32-arm64": {
        "npm_name": "@motyga/cli-win32-arm64",
        "npm_tag": "win32-arm64",
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "motyga": ["motyga", *MOTYGA_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "motyga": [],
    "motyga-linux-x64": [MOTYGA_PACKAGE_COMPONENT],
    "motyga-linux-arm64": [MOTYGA_PACKAGE_COMPONENT],
    "motyga-darwin-x64": [MOTYGA_PACKAGE_COMPONENT],
    "motyga-darwin-arm64": [MOTYGA_PACKAGE_COMPONENT],
    "motyga-win32-x64": [MOTYGA_PACKAGE_COMPONENT],
    "motyga-win32-arm64": [MOTYGA_PACKAGE_COMPONENT],
    "motyga-responses-api-proxy": ["motyga-responses-api-proxy"],
    "motyga-sdk": [],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in MOTYGA_PLATFORM_PACKAGES.items()
}

PACKAGE_CHOICES = tuple(PACKAGE_NATIVE_COMPONENTS)

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Motyga CLI npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="motyga",
        help="Which npm package to stage (default: motyga).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "motyga":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/motyga.js --version\n"
                    f"    node {staging_dir_str}/bin/motyga.js --help\n\n"
                )
            elif package == "motyga-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/motyga-responses-api-proxy.js --help\n\n"
                )
            elif package in MOTYGA_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="motyga-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "motyga":
        ...
        shutil.copy2(MOTYGA_CLI_ROOT / "bin" / "motyga.js", bin_dir / "motyga.js")

        readme_src = REPO_ROOT / "README.md"
        if readme_src.exists():
            shutil.copy2(readme_src, staging_dir / "README.md")

        # Apache-2.0 §4(a)/(d): ship LICENSE + NOTICE with the published fork.
        for legal_name in ("LICENSE", "NOTICE"):
            legal_src = REPO_ROOT / legal_name
            if legal_src.exists():
                shutil.copy2(legal_src, staging_dir / legal_name)

        package_json_path = MOTYGA_CLI_ROOT / "package.json"
    elif package in MOTYGA_PLATFORM_PACKAGES:
        platform_package = MOTYGA_PLATFORM_PACKAGES[package]
        ...
        with open(MOTYGA_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            motyga_package_json = json.load(fh)

        package_json = {
            "name": MOTYGA_NPM_NAME,
            "version": platform_version,
            "license": motyga_package_json.get("license", "Apache-2.0"),
            "os": [platform_package["os"]],
            "cpu": [platform_package["cpu"]],
            "files": ["vendor"],
            "repository": motyga_package_json.get("repository"),
        }

        engines = motyga_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = motyga_package_json.get("packageManager")
        if isinstance(package_manager, str):
            package_json["packageManager"] = package_manager
    elif package == "motyga-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "motyga-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "motyga-responses-api-proxy.js")
    ...
    elif package == "motyga-sdk":
        package_json_path = MOTYGA_SDK_ROOT / "package.json"
        stage_motyga_sdk_sources(staging_dir)
    else:
    ...
    if package == "motyga":
        package_json["files"] = ["bin/motyga.js", "LICENSE", "NOTICE"]
        package_json["optionalDependencies"] = {
            MOTYGA_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{MOTYGA_NPM_NAME}@"
                f"{compute_platform_package_version(version, MOTYGA_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["motyga"]
            if platform_package != "motyga"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[MOTYGA_NPM_NAME] = version
...
def stage_motyga_sdk_sources(staging_dir: Path) -> None:
    package_root = MOTYGA_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("motyga-sdk build did not produce a dist directory.")
...
def copy_native_binaries(
    ...
    for target_dir in vendor_src.iterdir():
        ...
        if MOTYGA_PACKAGE_COMPONENT in components_set:
        ...
        for component in sorted(components_set - {MOTYGA_PACKAGE_COMPONENT}):
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="motyga-npm-pack-") as pack_dir_str:
