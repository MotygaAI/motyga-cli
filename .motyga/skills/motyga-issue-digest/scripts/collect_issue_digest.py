...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a Motyga owner digest."
    )
    parser.add_argument(
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(motyga(?: desktop| app|\.app| cli)?|desktop|windows motyga app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
