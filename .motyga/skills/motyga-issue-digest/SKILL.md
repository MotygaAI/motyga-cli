---
name: motyga-issue-digest
description: Run a GitHub issue digest for openai/codex by feature-area labels, all areas, and configurable time windows. Use when asked to summarize recent Motyga bug reports or enhancement requests, especially for owner-specific labels such as tui, exec, app, or similar areas.
---
...
1. Run the collector from a current Motyga repo checkout:

```bash
python3 .motyga/skills/motyga-issue-digest/scripts/collect_issue_digest.py --labels tui exec --window-hours 24
```
...
```text
Use $motyga-issue-digest to run the Motyga issue digest for labels tui and exec over the previous 24 hours.
```

```text
Use $motyga-issue-digest to run the Motyga issue digest for all areas over the past week.
```
...
```bash
python3 .motyga/skills/motyga-issue-digest/scripts/collect_issue_digest.py --all-labels --window "past week" --limit-issues 10
```

Run the focused script tests:

```bash
pytest .motyga/skills/motyga-issue-digest/scripts/test_collect_issue_digest.py
```
