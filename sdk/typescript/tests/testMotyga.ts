...
import { Motyga } from "../src/motyga";
import type { MotygaConfigObject } from "../src/motygaOptions";

export const motygaExecPath =
  process.env.MOTYGA_EXEC_PATH ??
...
export function createTestClient(options: CreateTestClientOptions = {}): TestClient {
  ...
  return {
    cleanup: () => {},
    client: new Motyga({
      motygaPathOverride: motygaExecPath,
...
function mergeTestConfig(
  ...
  config: MotygaConfigObject | undefined,
): MotygaConfigObject | undefined {
  const mergedConfig: MotygaConfigObject | undefined =
  ...
  return {
    ...
    // sync does not race temp MOTYGA_HOME cleanup.
    features:
...
function hasExplicitProviderConfig(config: MotygaConfigObject | undefined): boolean {
...
function getCurrentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "MOTYGA_INTERNAL_ORIGINATOR_OVERRIDE") {
      continue;
    }
    if (value !== undefined) {
      env[key] = value;
