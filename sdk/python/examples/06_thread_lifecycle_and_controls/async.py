...
async def main() -> None:
    async with AsyncMotyga(config=runtime_config()) as motyga:
        ...
        reopened = await motyga.thread_resume(thread.id)
        listing_active = await motyga.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await motyga.thread_archive(reopened.id)
        listing_archived = await motyga.thread_list(limit=20, archived=True)
        unarchived = await motyga.thread_unarchive(reopened.id)

        resumed = await motyga.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await motyga.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (
