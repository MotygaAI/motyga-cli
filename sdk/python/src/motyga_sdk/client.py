...
from .errors import MotygaError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "motyga-cli-bin"
...
def _installed_motyga_path() -> Path:
    try:
        from motyga_cli_bin import bundled_motyga_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Motyga runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set MotygaConfig.motyga_bin "
            "explicitly."
        ) from exc

    return bundled_motyga_path()


def _installed_motyga_path_dirs() -> tuple[Path, ...]:
    try:
        from motyga_cli_bin import bundled_path_dir
...
class MotygaBinResolverOps:
    installed_motyga_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_motyga_bin_resolver_ops() -> MotygaBinResolverOps:
    return MotygaBinResolverOps(
        installed_motyga_path=_installed_motyga_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_motyga_bin(config: "MotygaConfig", ops: MotygaBinResolverOps) -> Path:
    if config.motyga_bin is not None:
        motyga_bin = Path(config.motyga_bin)
        if not ops.path_exists(motyga_bin):
            raise FileNotFoundError(
                f"Motyga binary not found at {motyga_bin}. Set MotygaConfig.motyga_bin "
                "to a valid binary path."
            )
        return motyga_bin

    return ops.installed_motyga_path()


def _resolve_motyga_bin(config: "MotygaConfig") -> Path:
    return resolve_motyga_bin(config, _default_motyga_bin_resolver_ops())


@dataclass(slots=True)
class MotygaConfig:
    """Configuration for launching and identifying the local Motyga runtime.

    Most callers can use ``Motyga()`` without configuration. Set ``motyga_bin``
    only when intentionally using a specific local Motyga executable.
    """

    motyga_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "motyga_python_sdk"
    client_title: str = "Motyga Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class MotygaClient:
    """Synchronous typed JSON-RPC client for `motyga app-server` over stdio."""

    def __init__(
        self,
        config: MotygaConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or MotygaConfig()
    ...
    def __enter__(self) -> "MotygaClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            motyga_bin = _resolve_motyga_bin(self.config)
            if self.config.motyga_bin is None:
                path_dirs = _installed_motyga_path_dirs()
            args = [str(motyga_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise MotygaError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Motyga process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Motyga process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise MotygaError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise MotygaError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_motyga_home() -> str:
    return str(Path.home() / ".motyga")
