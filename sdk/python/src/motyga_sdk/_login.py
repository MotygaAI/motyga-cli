...
from .client import MotygaClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncMotyga needed by async login handles."""

    _client: AsyncMotygaClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Motyga connection."""
        ...


def start_chatgpt_login(client: MotygaClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: MotygaClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Motyga.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Motyga.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncMotyga.login_chatgpt()`."""

    _motyga: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._motyga._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._motyga._ensure_initialized()
        return await self._motyga._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncMotyga.login_chatgpt_device_code()`."""
