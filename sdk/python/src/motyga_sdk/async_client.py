...
class AsyncMotygaClient:
    """Async wrapper around MotygaClient using thread offloading."""

    def __init__(self, config: MotygaConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = MotygaClient(config=config)

    async def __aenter__(self) -> "AsyncMotygaClient":
        """Start the Motyga process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Motyga process when leaving an async context."""
        await self.close()
        ...
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="motyga-goal-start-cleanup",
                    daemon=True,
                ).start()
