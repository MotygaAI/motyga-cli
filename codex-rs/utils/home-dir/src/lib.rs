...
/// specified by the `MOTYGA_HOME` environment variable (falling back to the
/// legacy `CODEX_HOME` for backward compatibility). If not set, defaults to
/// `~/.motyga`.
///
        ...
        Some(val) => {
            ...
            let metadata = std::fs::metadata(&path).map_err(|err| match err.kind() {
                std::io::ErrorKind::NotFound => std::io::Error::new(
                    ...
                    format!("MOTYGA_HOME points to {val:?}, but that path does not exist"),
                ),
                _ => std::io::Error::new(
                    err.kind(),
                    format!("failed to read MOTYGA_HOME {val:?}: {err}"),
                ),
            })?;
            ...
            if !metadata.is_dir() {
                Err(std::io::Error::new(
                    ...
                    format!("MOTYGA_HOME points to {val:?}, but that path is not a directory"),
                ))
            } else {
                let canonical = path.canonicalize().map_err(|err| {
                    std::io::Error::new(
                        err.kind(),
                        format!("failed to canonicalize MOTYGA_HOME {val:?}: {err}"),
                    )
                })?;
