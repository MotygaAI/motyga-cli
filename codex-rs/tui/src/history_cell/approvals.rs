        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " motyga to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
        ...
        TimedOut => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    "Review ".into(),
                    "timed out".bold(),
                    " before motyga could access ".into(),
                    Span::from(target).dim(),
                ],
            ),
        },
        Abort => match subject {
            ...
            ApprovalDecisionSubject::NetworkAccess { target } => (
                ...
                vec![
                    actor.subject().into(),
                    "canceled".bold(),
                    " the request for motyga network access to ".into(),
                    Span::from(target).dim(),
                ],
            ),
        },
...
pub fn new_guardian_timed_out_patch_request(files: Vec<String>) -> Box<dyn HistoryCell> {
    let mut summary = vec![
        "Review ".into(),
        "timed out".bold(),
        " before motyga could apply ".into(),
    ];
    if files.len() == 1 {
        summary.push("a patch touching ".into());
        summary.push(Span::from(files[0].clone()).dim());
