...
"""Verify that motyga-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the motyga-rs directory naming conventions.
...
MANIFEST_FEATURE_EXCEPTIONS = {
    ...
    "motyga-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under motyga-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `motyga-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `motyga-rs/<crate>/Cargo.toml` and "
        "`motyga-rs/utils/<crate>/Cargo.toml`."
    )
    print(
