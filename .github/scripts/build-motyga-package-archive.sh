...
usage() {
  cat <<'EOF'
Usage: build-motyga-package-archive.sh \
  --target <rust-target> \
  ...
  [--bwrap-bin <path>] \
  [--zsh-manifest <path>] \
  [--motyga-command-runner-bin <path>] \
  [--motyga-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --motyga-command-runner-bin)
      resource_args+=(
        --motyga-command-runner-bin
        "${2:?--motyga-command-runner-bin requires a value}"
      )
    ...
    --motyga-windows-sandbox-setup-bin)
      resource_args+=(
        --motyga-windows-sandbox-setup-bin
        "${2:?--motyga-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="motyga"
    entrypoint="motyga"
    archive_stem="motyga-package"
    ;;
  app-server)
    variant="motyga-app-server"
    entrypoint="motyga-app-server"
    archive_stem="motyga-app-server-package"
    ;;
  *)
    echo "No Motyga package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/motyga-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/motyga-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--motyga-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--motyga-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_motyga_package.py"
  --target "$target"
  --variant "$variant"
