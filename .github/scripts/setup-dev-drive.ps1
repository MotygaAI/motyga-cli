...
if (Test-Path "D:\") {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "motyga-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "MotygaDevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop
