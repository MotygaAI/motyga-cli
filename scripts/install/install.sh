...
RELEASE="${MOTYGA_RELEASE:-latest}"
NON_INTERACTIVE="${MOTYGA_NON_INTERACTIVE:-false}"

BIN_DIR="${MOTYGA_INSTALL_DIR:-$HOME/.local/bin}"
BIN_PATH="$BIN_DIR/motyga"
MOTYGA_HOME_DIR="${MOTYGA_HOME:-$HOME/.motyga}"
STANDALONE_ROOT="$MOTYGA_HOME_DIR/packages/standalone"
...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid Motyga release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
Environment:
  MOTYGA_RELEASE          Version to install; overridden by --release.
  MOTYGA_NON_INTERACTIVE  Set to 1, true, or yes to skip prompts.
EOF
        exit 0
        ;;
      *)
...
download_file() {
  ...
  echo "curl or wget is required to install Motyga." >&2
  exit 1
}
...
release_url_for_asset() {
  ...
  printf 'https://github.com/MotygaAI/motyga-cli/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}

release_metadata_url() {
  resolved_version="$1"

  printf 'https://api.github.com/repos/MotygaAI/motyga-cli/releases/tags/rust-v%s\n' "$resolved_version"
}
...
package_archive_digest() {
  ...
  if [ -z "$digest" ]; then
    echo "Could not find SHA-256 digest for $asset in motyga-package_SHA256SUMS." >&2
    exit 1
  fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Motyga download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Motyga archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Motyga." >&2
    exit 1
  fi
}
...
resolve_version() {
  ...
  release_json="$(download_text "https://api.github.com/repos/MotygaAI/motyga-cli/releases/latest")"
  resolved="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"rust-v\([^"]*\)".*/\1/p' | head -n 1)"

  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest Motyga release version." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> Motyga installer >>>"
  end_marker="# <<< Motyga installer <<<"
...
version_from_binary() {
  ...
  if [ ! -x "$motyga_path" ]; then
    return 1
  fi

  "$motyga_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/motyga" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/motyga" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_motyga() {
  command -v motyga 2>/dev/null || true
}

classify_existing_motyga() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && motyga"
      step "Future terminals: open a new terminal and run: motyga"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: motyga"
      step "Future terminals: open a new terminal and run: motyga"
      ;;
  esac
}

maybe_launch_motyga_now() {
  if prompt_yes_no "Start Motyga now?"; then
    step "Launching Motyga"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_motyga)"
  manager="$(classify_existing_motyga "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed Motyga at $existing_path"
  warn "Multiple managed Motyga installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask motyga"
      ;;
    bun)
      uninstall_cmd="bun remove -g @motyga/cli"
      ;;
    *)
      uninstall_cmd="npm uninstall -g @motyga/cli"
      ;;
  esac

  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Motyga now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Motyga. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Motyga installed. PATH order will determine which motyga runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 "$stage_release/bin/motyga" "$stage_release/motyga-path/rg"
  if [ -f "$stage_release/motyga-resources/bwrap" ]; then
    chmod 0755 "$stage_release/motyga-resources/bwrap"
  fi
  ln -sf "bin/motyga" "$stage_release/motyga"
...
install_legacy_platform_npm_release() {
  ...
  mkdir -p "$stage_release/motyga-resources" "$extract_dir"
  tar -xzf "$archive_path" -C "$extract_dir"

  cp "$vendor_root/motyga/motyga" "$stage_release/motyga"
  cp "$vendor_root/path/rg" "$stage_release/motyga-resources/rg"
  chmod 0755 "$stage_release/motyga" "$stage_release/motyga-resources/rg"
  if [ -f "$vendor_root/motyga-resources/bwrap" ]; then
    cp "$vendor_root/motyga-resources/bwrap" "$stage_release/motyga-resources/bwrap"
    chmod 0755 "$stage_release/motyga-resources/bwrap"
  fi
...
release_dir_is_complete() {
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/motyga-package.json" ] &&
        [ -x "$release_dir/bin/motyga" ] &&
        [ -x "$release_dir/motyga" ] &&
        [ -x "$release_dir/motyga-path/rg" ] ||
        return 1
      ;;
    legacy-platform-npm)
      [ -x "$release_dir/motyga" ] &&
        [ -x "$release_dir/motyga-resources/rg" ] ||
        return 1
      ;;
    *)
      return 1
  ...
  case "$layout:$expected_target" in
    package:*linux* | legacy-platform-npm:*linux*) [ -x "$release_dir/motyga-resources/bwrap" ] ;;
    *) true ;;
  esac
}
...
release_motyga_relative_path() {
  release_dir="$1"

  if [ -x "$release_dir/bin/motyga" ]; then
    printf 'bin/motyga\n'
  else
    printf 'motyga\n'
  fi
...
update_visible_command() {
  ...
  motyga_relative_path="$(release_motyga_relative_path "$release_dir")"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/$motyga_relative_path" "$tmp_link"
}
...
package_asset="motyga-package-$vendor_target.tar.gz"
checksum_asset="motyga-package_SHA256SUMS"
if release_asset_exists "$package_asset" "$resolved_version" &&
  ...
  asset="$package_asset"
elif release_asset_exists "motyga-npm-$npm_tag-$resolved_version.tgz" "$resolved_version"; then
  install_layout="legacy-platform-npm"
  asset="motyga-npm-$npm_tag-$resolved_version.tgz"
else
  echo "Could not find Motyga package or platform npm release assets for Motyga $resolved_version." >&2
  exit 1
fi
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Motyga CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Motyga CLI"
else
  step "Installing Motyga CLI"
fi
...
printf 'Motyga CLI %s installed successfully.\n' "$resolved_version"
maybe_launch_motyga_now
