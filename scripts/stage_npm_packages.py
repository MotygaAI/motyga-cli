#!/usr/bin/env python3
"""Stage one or more Motyga npm packages for release."""
...
WORKFLOW_NAME = ".github/workflows/release.yml"
GITHUB_REPO = "MotygaAI/motyga-cli"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("motyga_build_npm_package", BUILD_SCRIPT)
...
MOTYGA_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "MOTYGA_PLATFORM_PACKAGES", {})
MOTYGA_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "MOTYGA_PACKAGE_COMPONENT", "motyga-package"
)
...
BINARY_COMPONENTS = {
    "motyga-responses-api-proxy": BinaryComponent(
        artifact_prefix="motyga-responses-api-proxy",
        dest_dir="motyga-responses-api-proxy",
        binary_basename="motyga-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if MOTYGA_PACKAGE_COMPONENT in components:
        install_motyga_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = MOTYGA_PACKAGE_COMPONENT in components or any(
...
def install_motyga_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing Motyga package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_motyga_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_motyga_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"motyga-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in MOTYGA_PLATFORM_PACKAGES:
        platform = package.removeprefix("motyga-")
        return f"motyga-npm-{platform}-{version}.tgz"
