...
remote_path='~/code/motyga-sync'
local_exec_server_port="${MOTYGA_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${MOTYGA_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/motyga-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/motyga-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/motyga-sync"
remote_motyga_rs="$remote_repo_root/motyga-rs"

cd "${remote_motyga_rs}"
cargo build -p motyga-cli --bin motyga

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/motyga exec-server --listen ws://127.0.0.1:0 \
...
echo "Start motyga via: "
printf '  MOTYGA_EXEC_SERVER_URL=ws://127.0.0.1:%s motyga -C %q\n' \
