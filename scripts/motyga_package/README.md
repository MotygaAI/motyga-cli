# Motyga package builder

This package contains the implementation behind `scripts/build_motyga_package.py`.
...
The builder creates a canonical Motyga package directory:
...
│   ├── motyga-command-runner.exe          # Windows only
│   └── motyga-windows-sandbox-setup.exe   # Windows only
└── motyga-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Motyga release
...
`motyga` and `motyga-app-server`. The `version` field in `motyga-package.json` is
read from `[workspace.package].version` in `motyga-rs/Cargo.toml`.
...
- Windows targets: `motyga-command-runner` and `motyga-windows-sandbox-setup`,
...
`--motyga-command-runner-bin` plus `--motyga-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Motyga-built V8 release pair before invoking
...
DotSlash manifest at `scripts/motyga_package/rg`. Downloaded archives are cached
under `$TMPDIR/motyga-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/motyga_package/codex-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/motyga-package/<target>-zsh` and installed at
`motyga-resources/zsh/bin/zsh`. Pass `--zsh-manifest` to use a different
