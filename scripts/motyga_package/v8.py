"""Motyga-built V8 artifact overrides for package Cargo builds."""
...
def resolve_motyga_v8_cargo_env(
    spec: TargetSpec,
    *,
    ...
    artifacts = fetch_motyga_v8_artifacts(spec, cache_root=cache_root)
    return {
...
def fetch_motyga_v8_artifacts(
    ...
    *,
    ...
    if spec.is_windows:
        raise RuntimeError(
            f"No Motyga-built V8 release artifacts for target: {spec.target}"
        )
...
def resolved_v8_crate_version() -> str:
    ...
    cargo_lock = tomllib.loads((REPO_ROOT / "motyga-rs" / "Cargo.lock").read_text())
    versions = sorted(
        {
            package["version"]
...
def default_cache_root() -> Path:
    return Path(tempfile.gettempdir()) / "motyga-package"
...
def ensure_valid_artifact(artifact: Path, checksum: str, url: str) -> None:
    ...
    artifact.unlink(missing_ok=True)
    raise RuntimeError(
        f"Motyga-built V8 artifact {artifact} failed checksum validation."
    )
