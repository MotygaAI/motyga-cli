"""Canonical Motyga package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "motyga-resources"
    path_dir = package_dir / "motyga-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    if inputs.motyga_command_runner_bin is not None:
        copy_executable(
            inputs.motyga_command_runner_bin,
            resources_dir / "motyga-command-runner.exe",
            is_windows=True,
        )

    if inputs.motyga_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.motyga_windows_sandbox_setup_bin,
            resources_dir / "motyga-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    metadata = {
        ...
        "resourcesDir": "motyga-resources",
        "pathDir": "motyga-path",
    }
    write_json(package_dir / "motyga-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "motyga-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: motyga-package.json")
    ...
    required_files = [
        ...
        Path("motyga-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("motyga-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("motyga-resources") / "bwrap")
        executable_files.append(Path("motyga-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("motyga-resources") / "motyga-command-runner.exe",
                Path("motyga-resources") / "motyga-windows-sandbox-setup.exe",
            ]
        )
